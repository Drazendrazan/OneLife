...
AddEventHandler('Ouvre:taxi', function()
    ...
    if xPlayer.job.name == 'taxi' then
        for i=1, #xPlayers, 1 do
            ...
            TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Taxi', '~b~Annonce', 'Le taxi est désormais ~g~Ouvert~s~!', 'CHAR_FLOYD', 8)
        end
    end
end)
...
AddEventHandler('Ferme:taxi', function()
    ...
    if xPlayer.job.name == 'taxi' then
        for i=1, #xPlayers, 1 do
            local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
            TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Taxi', '~b~Annonce', 'Le taxi est désormais ~b~Fermer~s~!', 'CHAR_FLOYD', 8)
        end
    end
end)
