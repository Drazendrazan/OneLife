...
local open = false 
local mainMenu6 = RageUI.CreateMenu('', '~y~Véhicule')
         ...
         while open do 
            RageUI.IsVisible(mainMenu6,function() 
                ...
                onSelected = function()
                  ...
                  if IsPedSittingInAnyVehicle(playerPed) then
                    ...
                    if GetPedInVehicleSeat(vehicle, -1) == playerPed then
                      ...
                      ESX.ShowNotification('Met toi place conducteur, ou sortez de la voiture.')
                    end
                  else
                    local vehicle = ESX.Game.GetVehicleInDirection()
            
                    if DoesEntityExist(vehicle) then
                      ESX.ShowNotification('La voiture à été placé dans le garage.')
                      ESX.Game.DeleteVehicle(vehicle)
              ...
              RageUI.Separator("~h~↓ Véhicules ↓")
                ...
                RageUI.Button("Taxi classique", nil, {RightLabel = "→→"}, true , {
                    onSelected = function()
                  ...
                  RageUI.Button("Taxi Marseillais", nil, {RightLabel = "→→"}, true , {
                    onSelected = function()
                      local model = GetHashKey("406_taxi")
                      RequestModel(model)
                      while not HasModelLoaded(model) do Citizen.Wait(10) end
                      local pos = GetEntityCoords(PlayerPedId())
                      local vehicle = CreateVehicle(model, 920.74, -163.53, 74.85, 273.6657, true, true)
                  ...
                  RageUI.Button("Taxi RS7R", nil, {RightLabel = "→→"}, true , {
                    onSelected = function()
                      local model = GetHashKey("rs7rtaxi")
                      RequestModel(model)
                      while not HasModelLoaded(model) do Citizen.Wait(10) end
                      local pos = GetEntityCoords(PlayerPedId())
                      local vehicle = CreateVehicle(model, 920.74, -163.53, 74.85, 273.6657, true, true)
