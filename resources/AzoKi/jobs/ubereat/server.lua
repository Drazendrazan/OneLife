...
AddEventHandler('fUberEat:Ouvert', function()
  ...
  if xPlayer.job.name == ubereat then
    for i=1, #xPlayers, 1 do
      ...
      TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], ubereat, '~p~Informations', 'Le UberEat est ouvert', 'CHAR_STRIPPER_NIKKI', 2)
    end
  end
end)
...
AddEventHandler('fUberEat:Fermer', function()
  ...
  if xPlayer.job.name == ubereat then
    for i=1, #xPlayers, 1 do
      local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
      TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], ubereat, '~p~Informations', 'Le UberEat est fermé', 'CHAR_STRIPPER_NIKKI', 2)
    end
  end
end)
...
AddEventHandler('fUberEat:Perso', function(msg)
    ...
    if xPlayer.job.name == ubereat then
      for i=1, #xPlayers, 1 do
          local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
          TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], ubereat, '~p~Annonce', msg, 'CHAR_STRIPPER_NIKKI', 8)
      end
    end
end)
