...
local open = false 
local mainMenu = RageUI.CreateMenu('', 'Police') 
local subMenu1 = RageUI.CreateMenu('', 'Police') 
...
function OpenMenuAccueilPolice() 
    if open then 
        ...
        RageUI.Button("Appeler un policier a l'accueil", nil, {RightLabel = "→→"}, not codesCooldown5 , {
            onSelected = function()
            codesCooldown5 = true 
        TriggerServerEvent('Appel:police')
        ESX.ShowNotification('~g~Votre message a bien été envoyé aux policiers.')
        Citizen.SetTimeout(5000, function() codesCooldown5 = false end)    
        end

--[[
        RageUI.Button("Porter plainte", nil, {RightLabel = "→→"}, not codesCooldown5 , {
            onSelected = function()
            codesCooldown5 = true 
            local Msg = KeyboardInput("Message", '' , 200)
            TriggerServerEvent('Appel:plainte',Msg)
        ESX.ShowNotification('~g~Votre plainte a bien été envoyé aux policiers sur leur radio Discord.')
        Citizen.SetTimeout(5000, function() codesCooldown5 = false end)
]]
        ...
        RageUI.IsVisible(subMenu1, function()    
            ...
            RageUI.Button("Valider la Demande", "~o~Merci de respecter les policiers, tout abus sera sanctionné.", { Color = {BackgroundColor = { 76, 175, 80, 50}} }, true, {
                onSelected = function()
                    if (nomprenom == nil or nomprenom == '') then
                        ESX.ShowNotification('~b~Vous n\'a pas rempli ton Nom/Prénom')
                    elseif (numero == nil or numero == '') then
                        ESX.ShowNotification('~b~Vous n\'a pas rempli ton Numéro')
                    elseif (heurerdv == nil or heurerdv == '') then
                        ESX.ShowNotification('~b~Vous n\'a pas rempli l\'heure de Rendez-vous')
                    elseif (rdvmotif == nil or rdvmotif == '' or rdvmotif == "Motif") then
                        ESX.ShowNotification('~b~Vous n\'a pas rempli le motif du rendez-vous')
                else
                    RageUI.CloseAll()
...
Citizen.CreateThread(function()
    while true do 
        local wait = 750
            ...
            for k in pairs(cfg_police.Position.Accueil) do 
                local plyCoords = GetEntityCoords(GetPlayerPed(-1), false)
                local pos = cfg_police.Position.Accueil
                local dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, pos[k].x, pos[k].y, pos[k].z)
  
                if dist <= 2.0 then 
                    wait = 0
                    Visual.Subtitle(cfg_police.TextAccueil, 1)
...
local npc = {
	{hash="csb_trafficwarden", x = 442.6279138, y = -981.92144, z = 30.689573, a = 94.3937}, --Accueil
    {hash="s_m_y_cop_01", x = 480.34, y = -996.67, z = 30.69, a = 87.02}, --Armurerie
    {hash="s_m_y_cop_01", x = 415.01, y = -1024.37, z = 29.50, a = 178.09}, --Guitoune parking
}
