...
RegisterNetEvent('dok_handcuff:cbClosestPlayerID')
AddEventHandler('dok_handcuff:cbClosestPlayerID', function(wannacuff, method)
	...
	if distance ~= -1 and distance <= 3 and not IsPedInAnyVehicle(PlayerPedId()) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) and not isArresting then
		_TriggerServerEvent('dok_handcuff:handcuff', GetPlayerServerId(closestPlayer), wannacuff, method)
	else
		ESX.ShowNotification('Aucun joueur à proximité.')
	end
end)

RegisterNetEvent('dok_handcuff:thecuff')
AddEventHandler('dok_handcuff:thecuff', function(wannamove, cufferSource)
	...
	if wannamove then
		...
		_TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 3.0, 'cuff', 0.7)
		Citizen.Wait(2300)
	...
	elseif not wannamove then
		_TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 3.0, 'uncuff', 0.7)
...
RegisterNetEvent('dok_handcuff:arresting')
AddEventHandler('dok_handcuff:arresting', function()
	ESX.Streaming.RequestAnimDict(cuffdict)
...
RegisterNetEvent('dok_handcuff:unarresting')
AddEventHandler('dok_handcuff:unarresting', function()
