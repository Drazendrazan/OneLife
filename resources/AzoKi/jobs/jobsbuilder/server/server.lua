...
RegisterCommand('jobsbuilder', function(source, args)
    ...
    if xPlayer.getGroup() == '_dev' then
        AzoKiServerUtils.toClient('ewen:jobsbuilder', source)
    end
end)

RegisterServerEvent('AzoKi:CreateFarmEntreprise')
AddEventHandler('AzoKi:CreateFarmEntreprise', function(namejob, labeljob, namerecolteitem, labelrecolteitem, PositionRecolte, nametraitementitem, labeltraitementitem, PositionTraitement, PositionVente, PositionCoffreEntreprise)
    ...
    TriggerClientEvent('esx:showNotification', source, '~p~AzoKi ~w~~n~Le Job à été crée avec succès, Attendez le prochain reboot.')
end)
...
RegisterServerEvent('AzoKi:initFarmSociety')
AddEventHandler('AzoKi:initFarmSociety', function()
    TriggerClientEvent('AzoKi:SendEntrepriseFarmList', source, EntrepriseFarmList)
end)
		...
		if type == 1 then -- Recolte
			if ListeItemRecolt[itemRecolte] == nil then
				...
				if ListeItemRecolt[itemRecolte].name == itemRecolte then
					...
					if Quantity >= 70 then
						xPlayer.showNotification('~p~Erreur ~w~~n~Vous êtes trop lourd pour faire ceci')
					else
					...
					TriggerEvent('AzoKilpb_fantadmin:ban', source, source, 'Tricher est interdit ( Activité Légal )', 0)
				end
			end
		elseif type == 2 then -- Traitement
			if ListeItemRecolt[itemRecolte] == nil or ListeItemTraitement[ItemTraitement] == nil then
				...
				if ListeItemRecolt[itemRecolte] == itemRecolte or ListeItemTraitement[itemTraitement] == itemTraitement then
					...
					if Quantity <= 0 then
						xPlayer.showNotification('~p~Erreur ~w~~n~Vous n\'avez rien à traiter')
