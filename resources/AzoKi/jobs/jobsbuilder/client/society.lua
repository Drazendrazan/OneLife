...
AzoKi.Job ={
    OpenSocietyMenu = function(society, position)
        ...
        while menu do
            ...
            RageUI.IsVisible(menu, function()
                ...
                RageUI.Separator("~p~Société : ~s~"..society.label)
                if AzoKi.jobgrade == 'boss' then
                    RageUI.Separator("~p~Argent Liquide : ~s~"..money.."$")
                    --RageUI.Separator("~p~Argent Sale : ~s~"..moneysale.."$")
                    RageUI.Separator("")
                        ...
                        onSelected = function()
                            if List.ActionButton == 1 then 
                                ...
                                if string ~= "" then
                                    ...
                                    AzoKi.Job.OpenSocietyMenu({label = AzoKi.joblabel, name = AzoKi.job }, ActionPatron)
                                end
    ...
    BossMenu = function(entreprise)
        local main = RageUI.CreateMenu("", "~p~Gestion "..AzoKi.joblabel)
        ...
        while main do 
            ...
            RageUI.IsVisible(main, function()
                RageUI.Separator('Argent de la société : ~p~'.. AzoKi.societymoney)
                RageUI.Separator('')
                RageUI.Button('Retirer de l\'argent', nil, {}, true, {
                    onSelected = function() 
                        local string = KeyboardInput('Combien ?', '', 999)
                        if string ~= "" then
                            Montant = tonumber(string)
                        end
                        TriggerServerEvent('ewensociety:withdrawMoney', AzoKi.job, tonumber(Montant))
                    end
                });
                RageUI.Button('Deposer de l\'argent', nil, {}, true, {
                    onSelected = function() 
                        ...
                        TriggerServerEvent('ewensociety:depositMoney', AzoKi.job, tonumber(Montant))
                    end
                });
            end)
...
function OpenSocietyPlayerInventoryMenu(society, position)
    ...
    while menu do
        ...
        RageUI.IsVisible(menu, function()
            ...
            RageUI.Separator("~p~Que voulez-vous déposer ?")
            ESX.PlayerData = ESX.GetPlayerData()
            for i = 1, #ESX.PlayerData.inventory do
                if ESX.PlayerData.inventory[i].count > 0 then
                    RageUI.Button("• "..ESX.PlayerData.inventory[i].label, nil, { RightLabel = "Quantité(s) : ~p~x"..ESX.PlayerData.inventory[i].count }, true, {
                        onSelected = function()
                            if UpdateOnscreenKeyboard() == 0 then return end
                            local string = KeyboardInput('Combien voulez vous déposer ?', '', 100)
                            if string ~= "" then
                                ...
                                if ESX.PlayerData.inventory[i].count >= deposit then
                                    ...
                                    print('t')
                                    ESX.ShowNotification('~p~Erreur ~w~~n~Tu n\'as pas assez d\'objets sur toi')
                                end
                            end
                        end
                    })
        ...
        if not RageUI.Visible(menu) then
            menu = RMenu:DeleteType('menu', true)
            AzoKi.Job.OpenSocietyMenu({label = AzoKi.joblabel, name = AzoKi.job }, position)
        end
    end
end
...
function OpenSocietyInventoryMenu(society, position, data)
    ...
    while menu do
        Wait(0)
        RageUI.IsVisible(menu, function()
            RageUI.Separator("~p~Que voulez-vous prendre ?")
            for k,v in pairs(data) do
                RageUI.Button(v.label, nil, {RightLabel = "Quantité(s) : ~p~x"..v.count}, true, {
                    onSelected = function()
                        if UpdateOnscreenKeyboard() == 0 then return end
                        local string = KeyboardInput('Combien voulez vous prendre ?', '', 100)
                        if string ~= "" then
                            ...
                            if v.count >= deposit then
                                ...
                                ESX.ShowNotification('~p~Erreur ~w~~n~Il n\'y a pas assez d\'objets dans le coffre')
                            end
                        end
                    end
                })
