...
AddEventHandler('fBurger:Ouvert', function()
  ...
  if xPlayer.job.name == 'burger' then
    for i=1, #xPlayers, 1 do
      ...
      TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'burger', '~p~Informations', 'Le Burger est ouvert', 'CHAR_STRIPPER_NIKKI', 2)
    end
  end
end)
...
AddEventHandler('fBurger:Fermer', function()
  ...
  if xPlayer.job.name == 'burger' then
    for i=1, #xPlayers, 1 do
      local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
      TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'burger', '~p~Informations', 'Le Burger est fermé', 'CHAR_STRIPPER_NIKKI', 2)
    end
  end
end)
...
AddEventHandler('fBurger:Perso', function(msg)
    ...
    if xPlayer.job.name == 'burger' then
      for i=1, #xPlayers, 1 do
          local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
          TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'burger', '~p~Annonce', msg, 'CHAR_STRIPPER_NIKKI', 8)
      end
    end
end)
