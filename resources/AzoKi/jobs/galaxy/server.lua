...
AddEventHandler('fGalaxyClub:Ouvert', function()
  ...
  if xPlayer.job.name == 'galaxy' then
    for i=1, #xPlayers, 1 do
      ...
      TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'galaxy', '~p~Informations', 'Le GalaxyClub est ouvert', 'CHAR_STRIPPER_NIKKI', 2)
    end
  end
end)
...
AddEventHandler('fGalaxyClub:Fermer', function()
  ...
  if xPlayer.job.name == 'galaxy' then
    for i=1, #xPlayers, 1 do
      local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
      TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'galaxy', '~p~Informations', 'Le GalaxyClub est fermé', 'CHAR_STRIPPER_NIKKI', 2)
    end
  end
end)
...
AddEventHandler('fGalaxyClub:Perso', function(msg)
    ...
    if xPlayer.job.name == 'galaxy' then
      for i=1, #xPlayers, 1 do
          local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
          TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'galaxy', '~p~Annonce', msg, 'CHAR_STRIPPER_NIKKI', 8)
      end
    end
end)
