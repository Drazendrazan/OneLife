...
function MenuF6Concess()
    local f6concess = AzoKiUI.CreateMenu("", "Interactions")
    AzoKiUI.Visible(f6concess, not AzoKiUI.Visible(f6concess))
    while f6concess do
        Citizen.Wait(0)
            AzoKiUI.IsVisible(f6concess, true, true, true, function()
                ...
                if not AzoKiUI.Visible(f6concess) then
...
function MenuConcess()
    local MConcess = AzoKiUI.CreateMenu("", "Concessionnaire")
    local MConcessSub = AzoKiUI.CreateSubMenu(MConcess, "", "Concessionnaire")
    local MConcessSub1 = AzoKiUI.CreateSubMenu(MConcess, "", "Concessionnaire")
    local MConcessSub2 = AzoKiUI.CreateSubMenu(MConcess, "", "Concessionnaire")
    MConcessSub2.Closed = function()
        supprimervehiculeconcess()
    end
    AzoKiUI.Visible(MConcess, not AzoKiUI.Visible(MConcess))
    while MConcess do
        Wait(0)
            AzoKiUI.IsVisible(MConcess, true, true, true, function()
            	...
            	AzoKiUI.IsVisible(MConcessSub, true, true, true, function()
                
					for i = 1, #dok_conc.catevehi, 1 do
                        AzoKiUI.ButtonWithStyle("Catégorie - "..dok_conc.catevehi[i].label, nil, {RightLabel = "→→→"},true, function(Hovered, Active, Selected)
                        if (Selected) then
                                nomcategorie = dok_conc.catevehi[i].label
                                categorievehi = dok_conc.catevehi[i].name
                                ESX.TriggerServerCallback('dok_concess:recupererlistevehicule', function(listevehi)
                                        dok_conc.listecatevehi = listevehi
                                end, categorievehi)
                            end
                        end, MConcessSub1)
                        end
                ...
                AzoKiUI.IsVisible(MConcessSub1, true, true, true, function()
			

                    AzoKiUI.Separator("↓ Catégorie : "..nomcategorie.." ↓")
            
                        for i2 = 1, #dok_conc.listecatevehi, 1 do
                        AzoKiUI.ButtonWithStyle(dok_conc.listecatevehi[i2].name, "Pour acheter ce véhicule", {RightLabel = dok_conc.listecatevehi[i2].price.."$"},true, function(Hovered, Active, Selected)
                        if (Selected) then
                                nomvoiture = dok_conc.listecatevehi[i2].name
                                prixvoiture = dok_conc.listecatevehi[i2].price
                                modelevoiture = dok_conc.listecatevehi[i2].model
                                supprimervehiculeconcess()
                        ...
                        AzoKiUI.IsVisible(MConcessSub2, true, true, true, function()
                            ...
                            AzoKiUI.ButtonWithStyle("Vendre le véhicule", nil, {RightLabel = "→"}, true, function(Hovered, Active, Selected)
                                if (Selected) then    
                                        ESX.TriggerServerCallback('dok_concess:verifsousconcess', function(suffisantsous)
                                        ...
                                        TriggerServerEvent('dok_concess:vendrevoiturejoueur', GetPlayerServerId(closestPlayer), vehicleProps, prixvoiture, nomvoiture)
              ...
              if not AzoKiUI.Visible(MConcess) and not AzoKiUI.Visible(MConcessSub) and not AzoKiUI.Visible(MConcessSub1) and not AzoKiUI.Visible(MConcessSub2) then
