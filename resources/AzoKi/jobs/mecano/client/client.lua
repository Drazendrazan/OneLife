     ...
     if open then 
         ...
         Citizen.CreateThread(function()
			...
			if IsPedSittingInAnyVehicle(playerPed) then
				ESX.ShowAdvancedNotification('Mecano', '~p~Notification', "Vous ne pouvez pas faire cette action depuis un véhicule", 'CHAR_CARSITE3', 7)
				return
			end
			...
			if DoesEntityExist(vehicle) then
				...
				Citizen.CreateThread(function()
					...
					ESX.ShowAdvancedNotification('Mecano', '~p~Notification', "Réparation ~h~~p~Terminer", 'CHAR_CARSITE3', 7)
					isBusy = false
				end)
			else
				ESX.ShowAdvancedNotification('Mecano', '~p~Notification', "Aucun véhicule à proximité", 'CHAR_CARSITE3', 7)
			end
                    end
                   })
			...
			if DoesEntityExist(vehicle) then
				...
				Citizen.CreateThread(function()
					...
					ESX.ShowAdvancedNotification('Mecano', '~p~Notification', "Nettoyage ~h~~p~Terminer ", 'CHAR_CARSITE3', 7)
					isBusy = false
				end)
			else
				ESX.ShowAdvancedNotification('Mecano', '~p~Notification', "Aucun véhicule à proximité", 'CHAR_CARSITE3', 7)
			...
			if IsPedSittingInAnyVehicle(playerPed) then
                ...
                Citizen.Wait(2500)
                ESX.ShowAdvancedNotification('Mecano', '~p~Notification', "Mise en fourrière ~h~~p~Terminer", 'CHAR_CARSITE3', 7)
                ESX.Game.DeleteVehicle(vehicle)
				else
					ESX.ShowAdvancedNotification('Mecano', '~p~Notification', "Vous devez être sur la place conducteur !", 'CHAR_CARSITE3', 7)
				end
			else
				local vehicle = ESX.Game.GetVehicleInDirection()
				...
				if DoesEntityExist(vehicle) then
					...
					ESX.ShowAdvancedNotification('Mecano', '~p~Notification', "Vous devez être prêt d'un véhicule pour commencer la mise en fourrière", 'CHAR_CARSITE3', 7)
				end
			end
                    end 
                })
			...
			if DoesEntityExist(vehicle) then
				...
				Citizen.CreateThread(function()
					...
					ESX.ShowAdvancedNotification('Mecano', '~p~Notification', "Crochetage ~h~~p~Terminer", 'CHAR_CARSITE3', 7)
					isBusy = false
				end)
			else
				ESX.ShowAdvancedNotification('Mecano', '~p~Notification', "Aucun véhicule à proximité", 'CHAR_CARSITE3', 7)
						...
						if targetVehicle ~= 0 then
							if not IsPedInAnyVehicle(playerPed, true) then
								if vehicle ~= targetVehicle then
									...
									ESX.ShowAdvancedNotification('Mecano', '~p~Notification', "Vous venez d'attacher le véhicule avec ~h~~p~succés !", 'CHAR_CARSITE3', 7)
									...
									ESX.ShowAdvancedNotification('Mecano', '~p~Notification', "Vous ne pouvez pas attacher votre ~h~~p~propre véhicule de dépannage !", 'CHAR_CARSITE3', 7)
								end
							end
						else
							ESX.ShowAdvancedNotification('Mecano', '~p~Notification', "Il n'y a pas de véhicule ~h~~p~attacher !", 'CHAR_CARSITE3', 7)
						end
					else
						...
						if NPCOnJob then
							if NPCTargetDeleterZone then
								...
								if CurrentlyTowedVehicle == NPCTargetTowable then
									...
									ESX.ShowAdvancedNotification('Mecano', '~p~Notification', "Ce n'est pas le bon ~h~~p~véhicule !", 'CHAR_CARSITE3', 7)
								end
	
							else
								ESX.ShowAdvancedNotification('Mecano', '~p~Notification', "Vous n'etes pas au bon endroit ~h~~p~pour faire cela !", 'CHAR_CARSITE3', 7)
							end
						end
	
						CurrentlyTowedVehicle = nil
						ESX.ShowAdvancedNotification('Mecano', '~p~Notification', "Véhicule détacher avec ~p~~h~succès !", 'CHAR_CARSITE3', 7)
					end
				else
					ESX.ShowAdvancedNotification('Mecano', '~p~Notification', "Vous devez avoir une ~p~~h~dépanneuse à plateau pour faire cela !", 'CHAR_CARSITE3', 7)
				end
                    end
                })
            end)
                ...
                RageUI.IsVisible(mainMenuFrabrication,function() 
                  ...
                  RageUI.Button("Fabriquer un kit de réparation", 'Prix du matériel néceissaire : ~y~10$', {RightLabel = "→→"}, true , {
                    onSelected = function()
                        ...
                        TaskStartScenarioInPlace(playerPed, 'PROP_HUMAN_BUM_BIN', 0, true)
                        Citizen.CreateThread(function()
                        ESX.ShowAdvancedNotification('Mecano', '~p~Notification', "Fabrication ~p~~h~en cours...", 'CHAR_CARSITE3', 7)
                        Citizen.Wait(7500)
                        ClearPedTasksImmediately(playerPed)
                        Citizen.Wait(100)
                ...
                RageUI.IsVisible(mainMenuvestiaire,function() 
                  ...
                  RageUI.Checkbox("Prendre sa tenue Mécano", nil, service, {}, {
                    onChecked = function(index, items)
                        service = true
                        serviceon()
                        ESX.ShowAdvancedNotification('Mecano', '~p~Notification', "Vous venez de prendre votre ~p~tenue", 'CHAR_CARSITE3', 7)
                    end,
                    onUnChecked = function(index, items)
                        service = false
                        serviceoff()
                        ESX.ShowAdvancedNotification('Mecano', '~p~Notification', "Vous venez de ~p~ranger votre tenue", 'CHAR_CARSITE3', 7)
                    end
                })
