...
AzoKi.Ambulance.OpenGetItemSoins = function()
...
AzoKi.Ambulance.OpenAmbulanceMenu = function()
    local menu = RageUI.CreateMenu('', "Voici les appeles disponibles")
    local OpenInteractAmbulanceMenu = RageUI.CreateSubMenu(menu, "", 'Actions disponible')
    local OpenPubMenuAmbulance = RageUI.CreateSubMenu(menu, "", 'Actions disponible')
    RageUI.Visible(menu, not RageUI.Visible(menu))
    while menu do
        Citizen.Wait(0)
        RageUI.IsVisible(menu, function()
            RageUI.Button('Menu Interaction', nil, {RightLabel = '>'}, true, {
                onSelected = function() 
                end
            }, OpenInteractAmbulanceMenu)
            RageUI.Button('Menu Appel', nil, {RightLabel = '>'}, true, {
                onSelected = function() 
                    TriggerServerEvent('')
                end
            }, OpenReportListEms)
        ...
        RageUI.IsVisible(OpenInteractAmbulanceMenu, function()
            RageUI.Button('Réanimer', nil, {RightLabel = '>'}, true, {
                onSelected = function()
                    ...
                    if closestPlayer == -1 or closestDistance > 3.0 then
                        ESX.ShowNotification('~p~OneLife~w~ ~n~Aucun joueur au alentours.')
                    else
            ...
            RageUI.Button('Mettre une Facture', nil, {RightLabel = '>'}, true, {
                onSelected = function() 
                    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
                    if closestPlayer == -1 or closestDistance > 3.0 then
                        ESX.ShowNotification('~b~Erreur ~w~~n~Il n\'y a aucun joueur au alentours')
                    else
                        local string = KeyboardInput('Montant de la facture', '', 7)
                        if string ~= "" then
        ...
        RageUI.IsVisible(OpenPubMenuAmbulance, function()
            RageUI.Button('[Annonces] ~b~EMS ~g~Ouvert~s~', nil, {RightLabel = '>'}, true, {
                onSelected = function()
                    TriggerServerEvent('EMS:Ouvert')
                end
            })
            RageUI.Button('[Annonces] ~b~EMS ~r~Fermé~s~', nil, {RightLabel = '>'}, true, {
                onSelected = function()
                    TriggerServerEvent('EMS:Fermer')
                end
            })
            RageUI.Button('[Annonces] ~o~Recrutement ~b~EMS~s~', nil, {RightLabel = '>'}, true, {
                onSelected = function()
                    TriggerServerEvent('EMS:Recrutement')
                end
            })
            RageUI.Button('[Annonces] Message ~p~personnalisé~s~', nil, {RightLabel = '>'}, true, {
                onSelected = function()
                    local message = KeyboardInput('Veuillez écrire le messsage à envoyer', '', 160)
                    if message ~= nil and message ~= "" then
                    TriggerServerEvent('EMS:Annonceperso', message)
                    end
                end
            })
        end)
...
AzoKi.Ambulance.OpenVestiaire = function()
        ...
        if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' then
            ...
            if dist <= 2.0 then 
                ...
                if IsControlJustPressed(1,51) then
                    AzoKi.Ambulance.OpenGetItemSoins()
                end
            end
        end
    Citizen.Wait(wait)
...
cfg_ems = {
    ...
    TextRentreVehicule = "Appuyez sur ~p~[E] ~s~pour ranger le ~p~véhicule ~s~!",  -- Text Rentré Voiture
...
local open = false 
local mainMenu6 = RageUI.CreateMenu('', '~p~Véhicule')
                ...
                RageUI.Button(v.buttoname, nil, {RightLabel = "→→"}, true , {
                    onSelected = function()
                        if not ESX.Game.IsSpawnPointClear(vector3(v.spawnzone.x, v.spawnzone.y, v.spawnzone.z), 10.0) then
                            ESX.ShowNotification("~p~Onelife - Ambulance\n~p~Point de spawn bloqué")
                        else
...
local open = false 
local mainMenu6 = RageUI.CreateMenu('', '~p~Hélico')
mainMenu6.Display.Header = true 
mainMenu6.Closed = function()
  open = false
end
