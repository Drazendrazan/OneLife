    ...
    while menu do
        ...
        RageUI.IsVisible(OpenSelectedAppel, function()
            ...
            if ReportListTable[SrcSelected].status == 1 then
                ...
                StatusText = '~p~En Attente'
            end
            RageUI.Separator('Status : '..StatusText)
            RageUI.Separator('')
            if ReportListTable[SrcSelected].status == 0 then
                RageUI.Button('Prendre l\'appel','Permet de prendre l\'appel, Vos collègues seront informés', {RightLabel = '>'}, true, {
                    onSelected = function()
                        if not AppelEnCours then
                            ...
                            ESX.ShowNotification('~p~OneLife~w~~n~Tu as pris l\'appel N*'..AppelsSelected)
                            TriggerServerEvent('EMS:UpdateReport', SrcSelected, true) --> TRUE = PRENDRE
                        else
                            ESX.ShowNotification('~p~OneLife~w~~n~Vous avez déjà un appel en cours\nClôture le pour en reprendre un.')
                        end
                    end
                })
            else
                if GetPlayerServerId(PlayerId()) == ReportListTable[SrcSelected].EMS_SRC then
                    RageUI.Button('Informer le patient de votre arrivée',nil, {RightLabel = '>'}, true, {
                        onSelected = function()
                    ...
                    RageUI.Button('Terminer l\'appel',nil, {RightLabel = '>'}, true, {
                        onSelected = function()
                            AppelEnCours = false
                            RemoveBlip(blip)
                            ESX.ShowNotification('~p~OneLife~w~~n~Vous avez terminé l\'intervention N*'..AppelsSelected)
