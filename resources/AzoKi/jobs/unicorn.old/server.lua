...
AddEventHandler('fUnicorn:bar', function(item,price)
    ...
    --else
         --TriggerClientEvent('esx:showNotification', source, "Vous n'avez assez ~p~d\'argent")
    --end
end)
...
AddEventHandler('fUnicorn:Fermer', function()
  ...
  if xPlayer.job.name == 'unicorn' then
    for i=1, #xPlayers, 1 do
      local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
      TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Vanilla Unicorn', '~p~Informations', 'L\'Unicorn est fermé', 'CHAR_MP_STRIPCLUB_PR', 2)
    end
  end
end)
...
AddEventHandler('fUnicorn:Perso', function(msg)
    ...
    if xPlayer.job.name == 'unicorn' then
      for i=1, #xPlayers, 1 do
          local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
          TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Vanilla Unicorn', '~p~Annonce', msg, 'CHAR_MP_STRIPCLUB_PR', 8)
      end
    end
end)
