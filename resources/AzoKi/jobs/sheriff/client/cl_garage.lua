...
local open = false 
local mainMenu6 = RageUI.CreateMenu('', '~b~Véhicule')
                ...
                RageUI.Button(v.buttoname, v.disc, {RightLabel = "→→"}, true , {
                    onSelected = function()
                        if not ESX.Game.IsSpawnPointClear(vector3(v.spawnzone.x, v.spawnzone.y, v.spawnzone.z), 10.0) then
                            ESX.ShowNotification("~g~sheriff\n~b~Point de spawn bloquée")
                        else
...
local open = false 
local mainMenu6 = RageUI.CreateMenu('', '~b~Héhlico')
mainMenu6.Display.Header = true 
mainMenu6.Closed = function()
  open = false
end
...
RMenu.Add('sheriff', 'vehicle_option', RageUI.CreateMenu("", "~b~Option véhicule"))
                        ...
                        for i = 1,9 do
                            if DoesExtraExist(pVeh, i) then
                                if IsVehicleExtraTurnedOn(pVeh, i) then
                                    RageUI.Button("~b~Désactiver~s~ l'extra "..i, nil, {}, true, {
                                        onSelected = function()
