...
AddEventHandler('Ouvre:vigne', function()
	...
	if xPlayer.job.name == 'vigne' then
		for i=1, #xPlayers, 1 do
			...
			TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Vigne', '~p~Annonce', '~p~Les vigne sont ouvert ! Venez achetez votre vin et jus de raisin !', 'CHAR_LEST_MIKE_CONF', 8)
		end
	end
end)
...
AddEventHandler('Ferme:vigne', function()
	...
	if xPlayer.job.name == 'vigne' then
		for i=1, #xPlayers, 1 do
			local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
			TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Vigne', '~p~Annonce', '~p~Les vignes ferme pour le moment !', 'CHAR_LEST_MIKE_CONF', 8)
		end
	end
end)
...
AddEventHandler('Recru:vigne', function()
	...
	if xPlayer.job.name == 'vigne' then
		for i=1, #xPlayers, 1 do
			local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
			TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Vigne', '~p~Annonce', '~y~Recrutement en cours, rendez-vous au vigne !', 'CHAR_LEST_MIKE_CONF', 8)
		end
	end
end)
...
AddEventHandler('recolteraisin', function()
	...
	if #(coords - vigne) < ZoneSize / 2 then
		...
		DropPlayer(source, 'Erreur Vigneron, contactez un Staff')
	end
end)
	...
	if #(coords - vigne) < ZoneSize / 2 then
		if xPlayer.job.name == 'vigne' then
			if jus_raisin > 250 then
				TriggerClientEvent('esx:showNotification', source, '~p~Il semble que tu ne puisses plus porter de jus de raisin...')
			elseif raisin < 5 then
				TriggerClientEvent('esx:showNotification', source, '~p~Pas assez de jus de raisin pour traiter...')
			else
	...
	if #(coords - vigne) < ZoneSize / 2 then
		if xPlayer.job.name == 'vigne' then
			...
			if jus_raisin > 0 then
				...
				TriggerClientEvent('esx:showNotification', xPlayer.source, "~p~Vous avez gagner ~p~10$~p~ pour chaque vente d'un jus de raisin")
				TriggerClientEvent('esx:showNotification', xPlayer.source, "~p~La société gagne ~p~10$~p~ pour chaque vente d'un jus de raisin")
			else 
	...
	if #(coords - vigne) < ZoneSize / 2 then
		if xPlayer.job.name == 'vigne' then
			...
			if grand_cru > 0 then
				...
				TriggerClientEvent('esx:showNotification', xPlayer.source, "~p~Vous avez gagner ~p~20$~p~ pour chaque vente d'un grand cru")
				TriggerClientEvent('esx:showNotification', xPlayer.source, "~p~La société gagne ~p~20$~p~ pour chaque vente d'un grand cru")
			else 
				TriggerClientEvent('esx:showNotification', xPlayer.source, "Vous n'avez plus rien à vendre")
			end
		end
