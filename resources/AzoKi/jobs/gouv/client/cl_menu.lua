...
function OpenMenuGouv()
	if open then 
        ...
        RageUI.IsVisible(subMenu2,function()
            ...
            if closestPlayer ~= -1 and closestDistance < 3 then
                ...
                RageUI.Button("~p~Personne autour de toi !", nil, {}, false, {})
            end
  ...
  RageUI.IsVisible(subMenu,function()
	...
	RageUI.Separator("↓ ~p~Argent(s) sale(s) ~s~↓")
	for k,v  in pairs(ArgentSale) do
		RageUI.Button("Argent sale :", nil, {RightLabel = "~p~"..v.label.."$"}, true, {
			onSelected = function() 
				--local combien = KeyboardInput("Combien ?", '', 8)
				--if tonumber(combien) > v.amount then
					--RageUI.Popup({message = "~p~quantité invalide"})
				--else
	...
	RageUI.Separator("↓ ~p~Objet(s) ~s~↓")
	for k,v  in pairs(Items) do
		RageUI.Button(v.label, nil, {RightLabel = "~p~x"..v.right}, true, {
			onSelected = function() 
				--local combien = KeyboardInput("Combien ?", '', 8)
				--if tonumber(combien) > v.amount then
					--RageUI.Popup({message = "~p~quantité invalide"})
	...
	RageUI.Separator("↓ ~p~Arme(s) ~s~↓")

	for k,v  in pairs(Armes) do
		RageUI.Button(v.label, nil, {RightLabel = "com ~p~"..v.right.. " ~s~munitions"}, true, {
			onSelected = function() 
				--local combien = KeyboardInput("Combien ?", '', 8)
				--if tonumber(combien) > v.amount then
					--RageUI.Popup({message = "~p~quantité invalide"})
...
RageUI.IsVisible(subMenu1,function()
	...
	RageUI.Button("Annonce [~p~Ouvertures~s~]", nil, {RightLabel = "→"}, true , {
		onSelected = function()
			TriggerServerEvent('Ouvre:gouvernement')
		end
	})

	RageUI.Button("Annonce [~p~Fermetures~s~]", nil, {RightLabel = "→"}, true , {
		onSelected = function()
