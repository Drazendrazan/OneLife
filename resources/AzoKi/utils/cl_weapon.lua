				...
				if (weapons[newWeap] ~= nil) and (weapons[newWeap].parameters.onlyBag) then
					...
					if ListBags[bagIndex] ~= nil then
						...
						if DoesEntityExist(vehicle) then
							...
							ESX.ShowNotification("~p~Action Impossible~s~ : Vous devez disposer d'un sac ou être proche d'une voiture.")
							continue = false
						end
					end
				end
...
AddEventHandler('nwx:flashballed', function()
	wait = 2
	knockedOut = true
	ESX.ShowNotification("~p~Tu as été mis KO par une flashball !")
end)
...
RegisterNetEvent('AzoKi:proofSmoke')
AddEventHandler('AzoKi:proofSmoke', function(toBeProof)
