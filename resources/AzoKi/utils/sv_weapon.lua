...
AddEventHandler('explosionEvent', function(sender, eventData)
	if eventData.explosionType == 20 then -- lacrymo
		...
		for x=0,6,3 do
			for y=0,6,3 do
				Citizen.CreateThread(function()
					...
					TriggerEvent('xr_particles:sync', id, newPos, 'core', 'exp_grd_bzgas_smoke', true, 50)
				end)
			end
		end
		...
		timeouts[sender] = ESX.SetTimeout(50 * 1000, function()
			TriggerClientEvent('AzoKi:proofSmoke', sender, false)
			proofs[sender] = nil
			timeouts[sender] = nil
	...
	while true do
		for playerId,wasProofed in pairs(proofs) do
			...
			if wasProofed and not toBeProof then
				TriggerClientEvent('AzoKi:proofSmoke', playerId, false)
			elseif toBeProof and not wasProofed then
				TriggerClientEvent('AzoKi:proofSmoke', playerId, true)
			end
		end
