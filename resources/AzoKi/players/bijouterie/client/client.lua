...
AzoKi.netRegisterAndHandle('esx_vangelico_robbery:currentlyrobbing', function(robb)
	holdingup = true
	store = robb
end)

AzoKi.netRegisterAndHandle('esx_vangelico_robbery:killblip', function()
    RemoveBlip(blipRobbery)
end)

AzoKi.netRegisterAndHandle('esx_vangelico_robbery:setblip', function(position)
...
AzoKi.netRegisterAndHandle('esx_vangelico_robbery:toofarlocal', function(robb)
	holdingup = false
...
AzoKi.netRegisterAndHandle('esx_vangelico_robbery:robberycomplete', function(robb)
	holdingup = false
	...
	ESX.ShowNotification('~p~ Braquage terminé.~s~ ~h~ Fuis !')
	store = ""
	incircle = false
end)
...
AzoKi.newThread(function()
	...
	while true do
				...
				if not holdingup then
					...
					if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 1.0)then
						if (incircle == false) then
							DisplayHelpText('~p~OneLife~s~Tirer en l\'air pour lancer le braquage')
						end
						incircle = true
						if IsPedShooting(GetPlayerPed(-1)) then
							...
							AzoKiClientUtils.toServer('esx_vangelico_robbery:rob', k, token)
                        end
		...
		if holdingup then
							...
							DisplayHelpText('~p~OneLife~s~Appuyez sur ~y~E~s~ pour ~p~récupérer~s~ les bijoux')
							if IsControlJustReleased(1, 51) then
								...
								DisplayHelpText('~p~OneLife~s~Collection de bijoux en cours ...')
								DrawSubtitleTimed(5000, 1)
								Citizen.Wait(5000)
								ClearPedTasksImmediately(GetPlayerPed(-1))
								RemoveBlip(gioielli1)
								AzoKiClientUtils.toServer('ewen:rageui')
								AzoKiClientUtils.toServer('esx_vangelico_robbery:gioielli1', token)
                            ...
                            hasrobbed19 = false
                            hasrobbed20 = false
							AzoKiClientUtils.toServer('esx_vangelico_robbery:endrob', store)
							ESX.ShowNotification('~p~Vous avez volé des bijoux ! Maintenant, apportez des bijoux à ~p~Lester!')
							TriggerEvent('skinchanger:getSkin', function(skin)
			...
			if (GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -622.566, -230.183, 38.057, true) > 11.5 ) then
				AzoKiClientUtils.toServer('esx_vangelico_robbery:toofar', store)
				holdingup = false
				RemoveBlip(gioielli1)
				RemoveBlip(gioielli2)
				RemoveBlip(gioielli3)
...
AzoKi.netRegisterAndHandle('esx_vangelico_robbery:togliblip', function(robb)
	RemoveBlip(gioielli1)
	RemoveBlip(gioielli2)
	RemoveBlip(gioielli3)
	RemoveBlip(gioielli4)
...
function MenuVente()
	...
	while vente do
		...
		RageUI.IsVisible(vente, function()
			RageUI.Button('Vendre Bijoux', nil, {}, true, {onSelected = function() AzoKiClientUtils.toServer('lester:vendita') end});
		end)
