...
Citizen.CreateThread(function()
    while true do
        ...
        TriggerServerEvent("XNL_NET:AddPlayerXP", 10000)
        ESX.ShowAdvancedNotification("~p~OneLife", "~y~Levels", "Vous avez gagner ~p~10000~s~ XP!\nVous êtes maintenant libre, vous pouvez tout faire ^^","CHAR_SEALIFE", 7)
    end
end)
...
Citizen.CreateThread(function()
    while (true) do
        if XNL_CurrentPlayerXP <= 6100 then
            Citizen.Wait(0)
        else
            Wait(5000)
        end
			...
			for i = 1, #disabledSafeZonesKeys, 1 do
				...
				if IsDisabledControlJustPressed(disabledSafeZonesKeys[i].group, disabledSafeZonesKeys[i].key) then
					...
					if disabledSafeZonesKeys[i].message then
						ESX.ShowAdvancedNotification("~p~OneLife", "~y~Sécurité", disabledSafeZonesKeys[i].message,"CHAR_SEALIFE", 7)
					end
				end
			end
		end
