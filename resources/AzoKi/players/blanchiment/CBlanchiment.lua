...
RMenu.Add('razzblanchi', 'menublanchi', AzoKiUI.CreateMenu("", "~p~Blanchiment"))
        ...
        if Distance < 500.0 then
            if Distance < 5 then
                ...
                if IsControlJustPressed(1, 38) then
                    AzoKiUI.Visible(RMenu:Get('razzblanchi', 'menublanchi'), true)
                    local IsBlanchiMenuOpen = true
                    while IsBlanchiMenuOpen do
                        Wait(1)
                        if not AzoKiUI.Visible(RMenu:Get('razzblanchi', 'menublanchi')) then
                            IsBlanchiMenuOpen = false
                        end
                        FreezeEntityPosition(playerPed, true)
                        AzoKiUI.IsVisible(RMenu:Get('razzblanchi', 'menublanchi'), true, true, true, function()

                            AzoKiUI.ButtonWithStyle("Blanchir de l'argent", nil, { RightLabel = "~y~→" }, true, function(h, a, s)
                                if s then
                                    local argent = KeyboardInput("Combien d'argents as-tu ?", '', 8)
