	...
	MySQL.Async.fetchAll("SELECT * FROM vips WHERE identifier=@identifier",{
        ...
        if data[1] then
            if tonumber(data[1].expiration) <= os.time() then
                xPlayer.showNotification('~p~OneLife ~w~~n~Votre VIP a  expirer')
                ...
                xPlayer.showNotification('~p~OneLife ~w~~n~Votre VIP :\nPrendra fin dans ~p~'..math.floor(day).. ' ~w~jours et ~p~'..txthrs.."~w~ heure(s).")
...
RegisterNetEvent('OneLife:GetVIP')
AddEventHandler('OneLife:GetVIP',function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    if source == 0 then
        if args[1] then
            ...
            if not VipTable[TARGET.identifier] then
                ...
                TARGET.showNotification('~p~OneLife ~w~~n~Vous venez d\'obtenir un VIP')
