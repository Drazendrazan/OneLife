    ...
    while true do
        ...
        if dist < 150 then
            if IsPedSittingInVehicle(PlayerPedId(), KartingVehicle) then
                ...
                ESX.ShowNotification('~p~OneLife ~w~~n~Vous avez quitté la zone du karting, Votre véhicule à été saisie')
            end
        end
        if dist2 < 50 then
            if IsPedSittingInVehicle(PlayerPedId(), KartingVehicle) then
...
function OpenMenuPlayerKarting()
    ...
    while menu do
        ...
        if not ActivityKarting then
            RageUI.Separator('~p~OneLife ~w~Activité - Karting')
            ...
            RageUI.Separator('AzoKi Activité - Karting')
