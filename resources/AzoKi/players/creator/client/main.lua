...
AddEventHandler("ronflex:creator", function(player)
    ...
    SetCamParams(sexcam, -785.37609863281,340.000,217.850--[[ z float ]], 4.0 --[[ rotation x ]], 0.0 --[[ rotation y  float]], 0.215, 42.2442, 0, 1, 1, 2)
    ...
    while main do
        ...
        RageUI.IsVisible(main, function()
            ...
            RageUI.Button('Prénom', nil, {RightLabel = prenom}, true, {
                onSelected = function() 
                    local string = KeyboardInput('Entrer vos informations',  '', 999)
					if string ~= "" then
						prenom = string 
					end
                end
        ...
        while true do 
            ...
            if dist < 5 then
                ...
                if IsControlJustPressed(1,51) then
                    SetEntityCoordsNoOffset(PlayerPedId(), -785.46185302734,-1044.6628417969,12.980167388916)
                    SetEntityHeading(PlayerPedId(), 116.69)
                    TriggerServerEvent("ronflex:bucket", false)
                    ESX.ShowAdvancedNotification("~p~OneLife", "Création d'identité", "Bienvenue sur OneLife\nProfitez bien de votre aventure !", "CHAR_SEALIFE", 8)
                    DisplayRadar(true)
                    break
                end
            end
