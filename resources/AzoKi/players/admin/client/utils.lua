...
function NoClip(bool)
    ...
    if not isStaffMode then
        ESX.ShowNotification("~p~[Staff] ~s~Vous devez avoir le staff mode activé pour faire cela !")
        return
    end
    isNoClip = bool
    if isNoClip then
...
local function aimCheck(player)
    -- function to check ConfigAdminMenu value onAim. If it's off, then
