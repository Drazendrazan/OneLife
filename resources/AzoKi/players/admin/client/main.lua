...
AddEventHandler("adminmenu:receivewarn", function(reason)
    ESX.Scaleform.ShowFreemodeMessage('~p~Vous avez reçu un avertissement', '~p~'..reason, 5)
end)
...
RegisterCommand("+noclip-admin", function()
    if ESX.GetPlayerData()['group'] ~= 'user' and isStaffMode then
        ...
        ESX.ShowNotification("~p~OneLife\n~s~Vous n'êtes pas en mode staff !")
    end
end, false)
RegisterKeyMapping('+noclip-admin', 'Raccourci Noclip', 'keyboard', ';')
