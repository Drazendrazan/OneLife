...
local function statsSeparator()
    AzoKiUI.Separator("Connectés: ~g~" .. connecteds .. " ~p~|~s~ Staff en ligne: ~o~" .. staff)
end
        ...
        while menuOpen do
            ...
            AzoKiUI.IsVisible(RMenu:Get(cat, subCat("main")), true, true, true, function()
                ...
                if isStaffMode then
                    ...
                    AzoKiUI.ButtonWithStyle("~g~Activer le Staff Mode", nil, {}, not serverInteraction, function(_, _, s)
                        if s then
                            serverInteraction = true
                            IS_IN_STAFF = true
                ...
                AzoKiUI.Separator("↓ ~g~Assistance ~s~↓")
            ...
            AzoKiUI.IsVisible(RMenu:Get(cat, subCat("players")), true, true, true, function()
                ...
                AzoKiUI.List("Filtre alphabétique:", filterArray, filter, nil, {}, true, function(_, _, _, i)
                    filter = i
                end)
                AzoKiUI.Separator("↓ ~g~Joueurs ~s~↓")
            ...
            AzoKiUI.IsVisible(RMenu:Get(cat, subCat("reports")), true, true, true, function()
                shouldStayOpened = true
                statsSeparator()
                AzoKiUI.Separator("↓ ~g~Paramètres ~s~↓")
                ...
                for sender, infos in pairs(localReportsTable) do
                    if infos.taken then
                        if hideTakenReports == false then
                            AzoKiUI.ButtonWithStyle(getIsTakenDisplay(infos.taken) .. "[~p~" .. infos.id .. "~s~] " .. cVarLong() .. "→ ~s~" .. infos.name, "~g~Créé il y a~s~: "..infos.timeElapsed[1].."m"..infos.timeElapsed[2].."h~n~~p~ID Unique~s~: #" .. infos.id .. "~n~~y~Description~s~: " .. infos.reason .. "~n~~o~Pris en charge par~s~: " .. infos.takenBy, { RightLabel = "→→" }, true, function(_, _, s)
                                if s then
                        ...
                        AzoKiUI.ButtonWithStyle(getIsTakenDisplay(infos.taken) .. "[~p~" .. infos.id .. "~s~] " .. cVarLong() .. "→ ~s~" .. infos.name, "~g~Créé il y a~s~: "..infos.timeElapsed[1].."m"..infos.timeElapsed[2].."h~n~~p~ID Unique~s~: #" .. infos.id .. "~n~~y~Description~s~: " .. infos.reason, { RightLabel = "→→" }, true, function(_, _, s)
                            if s then
                                selectedReport = sender
                            end
                        end, RMenu:Get(cat, subCat("reports_take")))
            ...
            AzoKiUI.IsVisible(RMenu:Get(cat, subCat("param")), true, true, true, function()
                shouldStayOpened = true
                statsSeparator()
                AzoKiUI.Separator("↓ ~g~Personnel ~s~↓")
                if ESX.GetPlayerData()['group'] ~= "user" then
                ...
                if ESX.GetPlayerData()['group'] ~= "user" then
                    AzoKiUI.Checkbox(cVarLong() .. "→ ~s~Affichage des Noms des joueurs", nil, isNameShown, { Style = AzoKiUI.CheckboxStyle.Tick }, function(Hovered, Selected, Active, Checked)
                        isNameShown = Checked;
                    end, function()
                        showNames(true)
                    end, function()
                ...
                if ESX.GetPlayerData()['group'] ~= "user" then
                    AzoKiUI.Checkbox(cVarLong() .. "→ ~s~Activer le Delgun", nil, isDelgunEnabled, { Style = AzoKiUI.CheckboxStyle.Tick }, function(Hovered, Selected, Active, Checked)
                ...
                if ESX.GetPlayerData()['group'] ~= "user" then
                    AzoKiUI.Checkbox(cVarLong() .. "→ ~s~Coordonnées", nil, isCoordsEnabled, { Style = AzoKiUI.CheckboxStyle.Tick }, function(Hovered, Selected, Active, Checked)
                        isCoordsEnabled = Checked;
                    end, function()
                        ToggleCoords()
                    end, function()
                        ToggleCoords()
                ...
                if ESX.GetPlayerData()['group'] == "_dev" then
                    AzoKiUI.Separator("↓ ~g~Te changer ~s~↓")
                    AzoKiUI.ButtonWithStyle(cVarLong() .. "→ ~s~Normal", nil, { RightLabel = "→→" }, canUse("ped", permLevel), function(_, _, s)
                        if s then
                            getBaseSkin()
                        end
                    end)
                    AzoKiUI.ButtonWithStyle(cVarLong() .. "→ ~p~Staff homme", nil, { RightLabel = "→→" }, canUse("ped", permLevel), function(_, _, s)
                        if s then
                            RequestModel("a_m_m_eastsa_02")
                            while not HasModelLoaded("a_m_m_eastsa_02") do
                                Wait(100)
                            end
                            SetPlayerModel(PlayerId(), "a_m_m_eastsa_02")
                            SetModelAsNoLongerNeeded("a_m_m_eastsa_02")
                        end
                    end)
                    AzoKiUI.ButtonWithStyle(cVarLong() .. "→ ~p~Staff femme", nil, { RightLabel = "→→" }, canUse("ped", permLevel), function(_, _, s)
                        if s then
                            RequestModel("a_f_m_prolhost_01")
                            while not HasModelLoaded("a_f_m_prolhost_01") do
                                Wait(100)
                            end
                            SetPlayerModel(PlayerId(), "a_f_m_prolhost_01")
                            SetModelAsNoLongerNeeded("a_f_m_prolhost_01")
                        end
                    end)
                    --[[
            ...
            AzoKiUI.IsVisible(RMenu:Get(cat, subCat("reports_take")), true, true, true, function()
                ...
                if localReportsTable[selectedReport] ~= nil then
                    ...
                    AzoKiUI.Separator("↓ ~g~Actions sur le report ~s~↓")
            ...
            AzoKiUI.IsVisible(RMenu:Get(cat, subCat("playersManage")), true, true, true, function()
                ...
                if not localPlayers[selectedPlayer] then
                    ...
                    AzoKiUI.Separator("↓ ~g~Téléportation ~s~↓")
                    ...
                    AzoKiUI.Separator("↓ ~y~Modération ~s~↓")
                    if (IN_SPECTATE) then
                        AzoKiUI.ButtonWithStyle(cVarLong() .. "→ ~s~Spectate", nil, { RightLabel = "~g~Activé" }, true, function(_, _, s)
                            if s then
                    ...
                    AzoKiUI.Separator("↓ ~g~Apparitions véhicules ~s~↓")

                    AzoKiUI.ButtonWithStyle(cVarLong() .. "→ ~s~BMX", nil, { RightLabel = "→→" }, true, function(Hovered, Active, Selected)
                        if Selected then
                            local veh = "bmx"
                            if veh ~= nil then
                    ...
                    AzoKiUI.ButtonWithStyle(cVarLong() .. "→ ~s~Sanchez", nil, { RightLabel = "→→" }, true, function(Hovered, Active, Selected)
                        if Selected then
                            local veh = "sanchez"
                            if veh ~= nil then
                            local model = GetHashKey(veh)
                    ...
                    AzoKiUI.ButtonWithStyle(cVarLong() .. "→ ~s~Panto", nil, { RightLabel = "→→" }, true, function(Hovered, Active, Selected)
                        if Selected then
                            local veh = "panto"
                            if veh ~= nil then
                                local model = GetHashKey(veh)
                    ...
                    AzoKiUI.ButtonWithStyle(cVarLong() .. "→ ~s~Sultan", nil, { RightLabel = "→→" }, true, function(Hovered, Active, Selected)
                        if Selected then
                            local veh = "sultan"
                            if veh ~= nil then
                                local model = GetHashKey(veh)
                    ...
                    AzoKiUI.ButtonWithStyle(cVarLong() .. "→ ~s~Give de l'argent (~g~liquide~s~)", nil, { RightLabel = "→→" }, canUse("giveMoney", permLevel), function(_, _, s)
                        if s then
                    ...
                    AzoKiUI.ButtonWithStyle(cVarLong() .. "→ ~s~Give de l'argent sale (~g~liquide~s~)", nil, { RightLabel = "→→" }, canUse("giveMoney", permLevel), function(_, _, s)
                        if s then
                            local qty = input("Quantité", "", 20, true)
                            if qty ~= nil then
                                msg("~y~Don de l'argent au joueur...")
            ...
            AzoKiUI.IsVisible(RMenu:Get(cat, subCat("items")), true, true, true, function()
                ...
                AzoKiUI.Separator("↓ ~g~Items disponibles ~s~↓")
            ...
            AzoKiUI.IsVisible(RMenu:Get(cat, subCat("setGroup")), true, true, true, function()
                ...
                statsSeparator()
                AzoKiUI.Separator("Gestion: ~y~" .. localPlayers[selectedPlayer].name .. " ~s~(~o~" .. selectedPlayer .. "~s~)")
                AzoKiUI.Separator("↓ ~g~Rangs disponibles ~s~↓")
            ...
            AzoKiUI.IsVisible(RMenu:Get(cat, subCat("setjobMenu")), true, true, true, function()
                ...
                AzoKiUI.Separator("↓ ~g~Jobs disponibles ~s~↓")
