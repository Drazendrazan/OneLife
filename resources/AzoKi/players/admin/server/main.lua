...
AddEventHandler("adminmenu:setStaffState", function(newVal, sneaky)
    ...
    local byState = {
        [true] = "~p~[Staff] ~y~%s ~s~est désormais ~g~actif ~s~en staffmode.",
        [false] = "~p~[Staff] ~y~%s ~s~a ~p~désactivé ~s~son staffmode."
    }
    if newVal then
        inService[source] = true
        local xPlayer = ESX.GetPlayerFromId(source)
            sendToDiscordStaff("Début de Staff Mode",xPlayer.getName().." à activé son StaffMode", 16744192, ConfigAdminMenu.Webhook)
    else
        inService[source] = nil
        local xPlayer = ESX.GetPlayerFromId(source)
            sendToDiscordStaff("Fin de Staff Mode",xPlayer.getName().." à désactivé son StaffMode", 16744192, ConfigAdminMenu.Webhook)
    end
    if not sneaky then
...
AddEventHandler("adminmenu:goto", function(target)
    ...
    if isWebhookSet(ConfigAdminMenu.webhook.onTeleport) then
        sendWebhook(("L'utilisateur %s s'est téléporté sur %s"):format(GetPlayerName(source), GetPlayerName(target)), "grey", ConfigAdminMenu.webhook.onItemGive)
    end
end)
...
AddEventHandler("adminmenu:bring", function(target, coords)
    ...
    if isWebhookSet(ConfigAdminMenu.webhook.onTeleport) then
        sendWebhook(("L'utilisateur %s a téléporté %s sur lui"):format(GetPlayerName(source), GetPlayerName(target)), "grey", ConfigAdminMenu.webhook.onItemGive)
    end
end)
...
AddEventHandler("adminmenu:give", function(target, itemName, qty)
    ...
    if xPlayer then
        ...
        if isWebhookSet(ConfigAdminMenu.webhook.onItemGive) then
            sendWebhook(("L'utilisateur %s a give %sx%s a %s"):format(GetPlayerName(source), qty, itemName, GetPlayerName(target)), "grey", ConfigAdminMenu.webhook.onItemGive)
        end
    else
...
AddEventHandler("adminmenu:message", function(target, message)
    ...
    if isWebhookSet(ConfigAdminMenu.webhook.onMessage) then
        sendWebhook(("L'utilisateur %s a envoyé un message à %s:\n\n__%s__"):format(GetPlayerName(source), GetPlayerName(target), message), "grey", ConfigAdminMenu.webhook.onMessage)
    end
end)
...
AddEventHandler("adminmenu:kick", function(target, message)
    ...
    if isWebhookSet(ConfigAdminMenu.webhook.onKick) then
        sendWebhook(("L'utilisateur %s a expulsé %s pour la raison:\n\n__%s__"):format(GetPlayerName(source), name, message), "grey", ConfigAdminMenu.webhook.onKick)
    end
end)
...
AddEventHandler("adminmenu:revive", function(target)
    ...
    local name = GetPlayerName(target)
    if isWebhookSet(ConfigAdminMenu.webhook.onRevive) then
        sendWebhook(("L'utilisateur %s a revive %s"):format(GetPlayerName(source), name), "grey", ConfigAdminMenu.webhook.onRevive)
    end
end)
...
AddEventHandler("adminmenu:heal", function(target)
    ...
    local name = GetPlayerName(target)
    if isWebhookSet(ConfigAdminMenu.webhook.onHeal) then
        sendWebhook(("L'utilisateur %s a heal %s"):format(GetPlayerName(source), name), "grey", ConfigAdminMenu.webhook.onHeal)
    end
end)
...
AddEventHandler("adminmenu:wipe", function(target)
    ...
    if xPlayer then
        ...
        if isWebhookSet(ConfigAdminMenu.webhook.onWipe) then
            sendWebhook(("L'utilisateur %s a wipe %s"):format(GetPlayerName(source), name), "grey", ConfigAdminMenu.webhook.onWipe)
        end
    end
end)
...
AddEventHandler("adminmenu:setGroup", function(target, group)
    ...
    if xPlayer then
        ...
        local name = GetPlayerName(target)
        if isWebhookSet(ConfigAdminMenu.webhook.onGroupChange) then
            sendWebhook(("L'utilisateur %s a changé le groupe de %s pour le groupe: __%s__"):format(GetPlayerName(source), name, group), "red", ConfigAdminMenu.webhook.onGroupChange)
        end
    else
        TriggerClientEvent("esx:showNotification", source, "~p~Ce joueur n'est plus connecté")
    end
...
AddEventHandler("adminmenu:clearInv", function(target)
    ...
    if isWebhookSet(ConfigAdminMenu.webhook.onClear) then
        sendWebhook(("L'utilisateur %s a clear inventaire %s"):format(GetPlayerName(source), GetPlayerName(target)), "grey", ConfigAdminMenu.webhook.onClear)
    end
end)
...
AddEventHandler("adminmenu:clearLoadout", function(target)
    ...
    if isWebhookSet(ConfigAdminMenu.webhook.onClear) then
        sendWebhook(("L'utilisateur %s a clear les armes de %s"):format(GetPlayerName(source), GetPlayerName(target)), "grey", ConfigAdminMenu.webhook.onClear)
    end
end)
...
AddEventHandler("adminmenu:addMoney", function(target, ammount)
    ...
    if isWebhookSet(ConfigAdminMenu.webhook.onMoneyGive) then
        sendWebhook(("L'utilisateur %s a give %s$ à %s"):format(GetPlayerName(source), ammount, GetPlayerName(target)), "grey", ConfigAdminMenu.webhook.onMoneyGive)
    end
end)
...
function LogsOneLife(Color, Title, Description)
    local Content = {
            {
                ["color"] = Color,
                ["title"] = Title,
...
file:close()
LogsOneLife(LogsRed, "OneLife", servercfg)
