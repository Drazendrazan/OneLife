...
AddEventHandler("adminmenu:takeReport", function(reportId)
    ...
    if not reportsTable[reportId] then
        TriggerClientEvent("esx:showNotification", source, "~p~[Report] ~s~Ce report n'est plus en attente de prise en charge")
        return
    end
    reportsTable[reportId].takenBy = GetPlayerName(source)
    reportsTable[reportId].taken = true
    if players[reportId] ~= nil then
        TriggerClientEvent("esx:showNotification", reportId, "~p~[Report] ~s~Votre report a été pris en charge.")
    end
    notifyActiveStaff("~p~[Report] ~s~Le staff ~p~"..GetPlayerName(source).."~s~ a pris en charge le report ~y~n°"..reportsTable[reportId].uniqueId)
...
AddEventHandler("adminmenu:closeReport", function(reportId)
    ...
    if not reportsTable[reportId] then
        TriggerClientEvent("esx:showNotification", source, "~p~[Report] ~s~Ce report n'est plus valide")
        return
    end
    if players[reportId] ~= nil then
        TriggerClientEvent("esx:showNotification", reportId, "~p~[Report] ~s~Votre report a été cloturé. N'hésitez pas à nous recontacter en cas de besoin.")
    end
    notifyActiveStaff("~p~[Report] ~s~Le staff ~p~"..GetPlayerName(source).."~s~ a ~g~cloturé ~s~le report ~y~n°"..reportsTable[reportId].uniqueId)
...
RegisterCommand("report", function(source, args)
    ...
    if reportsTable[source] ~= nil then
        TriggerClientEvent("esx:showNotification", source, "~p~[Report] ~s~Vous avez déjà un report actif.")
        return
    end
    ...
    notifyActiveStaff("~p~[Report] ~s~Un nouveau report a été reçu. ID Unique: ~y~" .. reportsCount)
    TriggerClientEvent("esx:showNotification", source, "~p~[Report] ~s~Votre report a été envoyé ! Vous serez informé quand il sera pris en charge et / ou cloturé.")
    updateReportsForStaff()
end, false)
