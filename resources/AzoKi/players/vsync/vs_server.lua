...
admins = {
    --'steam:1100001443b3518', -- Tewnya
    --'steam:11000014563b997', -- Malouxy
    --'steam:1100001444d5716', -- Enzo
    --'license:bd5809738a764c09b2db4f2ac7b1055f4e2526b8' -- Lena#lapute
...
RegisterCommand('freezetime', function(source, args)
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if freezeTime then
                --TriggerClientEvent('vSync:notify', source, 'Time is now ~p~frozen~s~.')
            else
    ...
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if not DynamicWeather then
                --TriggerClientEvent('vSync:notify', source, 'Changements métérologiques dynamiques ~p~desactivé~s~.')
            else
                --TriggerClientEvent('vSync:notify', source, 'Changements métérologiques dynamiques ~p~activé~s~.')
            end
        else
            TriggerClientEvent('chatMessage', source, '', {255,255,255}, '^8Error: ^1Vous n\'etes pas autorisé a utiliser cette commande.')
        end
    ...
    if source == 0 then
        ...
        if isAllowedToChange(source) then
            ...
            if blackout then
                ...
                --TriggerClientEvent('vSync:notify', source, 'Blackout is now ~p~disabled~s~.')
            end
            TriggerEvent('vSync:requestSync')
        end
    end
