...
-- GENERAL --
Config.webhook = 'https://discord.com/api/webhooks/986817599700631602/JfP2j8Chgh_rrqqGy2SoAM_rjVZvzpuiCpbAXPP7bL-HXyYr02zAgJblSwUflyfs8ZFQ'
...
AddEventHandler('BanSql:ICheatClient', function(reason)
	...
	if _source then
		...
		if name then
			...
			AddBan(_source, licenseid, playerip, name, 'Anti-Cheat OneLife', 0, reason, 1)
			DropPlayer(_source, ('Vous êtes banni de Onelife\nRaison : %s\nTemps Restant : Permanent\nAuteur : Anti-Cheat Onelife'):format(reason))
		end
	else
		print('BanSql Error : Anti-Cheat Onelife have received invalid id.')
	end
end)
...
AddEventHandler('BanSql:ICheatServer', function(target, reason)
	...
	if target then
		...
		if name then
			...
			AddBan(target, licenseid, playerip, name, 'Anti-Cheat Onelife', 0, reason, 1)
			DropPlayer(target, ('Vous êtes banni de Onelife\nRaison : %s\nTemps Restant : Permanent\nAuteur : Anti-Cheat Onelife'):format(reason))
		end
	else
		print('BanSql Error : Anti-Cheat OneLife have received invalid id.')
	end
