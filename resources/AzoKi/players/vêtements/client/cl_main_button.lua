                ...
                onSelected = function()  
                    ESX.TriggerServerCallback("ronflex:cbmoneytenue", function(cb)
                        if cb == true then 
                            ...
                            ESX.ShowNotification("~p~OneLife~w~ Vous ne disposez pas des fonds nécéssaires")
                        end
                    end)
                end
            })

            RageUI.Button("> Valider la tenue et l'enregister", nil, {BackgroundColor = {R = 0, G = 255, B = 0, A = 255}, RightLabel = '~p~750 $'}, true, {
                onSelected = function()
                    local name = KeyboardInput("Indiquer le nom de la tenue", "", 10)
                    if name then 
                        local TempoSkin = {}
                        local ListVet = {
                            ["tshirt_1"] = true,
                            ...
                            onSelected = function(Index)
                                ...
                                elseif Index == 2 then 
                                    TriggerEvent("skinchanger:getSkin", function(skin)
                                        ...
                                        TriggerServerEvent("esx_skin:save", skin)
                                        RageUI.CloseAll()
                                        ESX.ShowNotification("~p~OneLife~s~~n~Vous avez équiper votre tenue "..v.label.."")
                                    end)
                                elseif Index == 3 then
