...
function OpenMenuBarberShop(caca)
	...
	while menu do
		...
		RageUI.IsVisible(menu, function()
            ...
            RageUI.Button("Payer la coupe", nil, {RightLabel = "~p~100 $"}, true, {
                onSelected = function()
                    ESX.TriggerServerCallback("barber:getmoney", function(cb)
                        if cb == true then
                            buy = true
                            ESX.ShowAdvancedNotification("~p~OneLife Information", "Barber Shop", "Vous avez payé votre coupe avec succès !\nA bientôt", "CHAR_SEALIFE", 8)
                            ...
                            ESX.ShowAdvancedNotification("~p~OneLife Information", "Barber Shop", "Vous ne disposez pas des fonds nécéssaires !", "CHAR_SEALIFE", 8)
