...
function timer()
    ...
    while GetGameTimer() - T < 1000 do
      Wait(0)
      Draw2DText(0.4, 0.4, ("~s~C'est parti ~w~!"), 3.0)
    end
...
AddEventHandler('evy_arm:start_cl', function()
  ...
  if place == 1 then
    ...
    if grade >= 0.90 then
      ...
      notify("~p~" .. text[globalConfig.language]['win'])
    elseif grade <= 0.10 then
      PlayAnim(PlayerPedId(), "mini@arm_wrestling", "win_a_ped_b", 2)
      notify("~p~" .. text[globalConfig.language]['lose'])
    end
    Wait(4000)
...
AddEventHandler('evy_arm:check_cl', function(args)
  ...
  elseif args == 'noplace' then
    ...
    notify('~p~' ..text[globalConfig.language]['full'])
