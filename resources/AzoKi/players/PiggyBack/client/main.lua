...
RegisterCommand('porter', function(source, args)
	if not GetSafeZone() then
		...
		ESX.ShowNotification('~p~OneLife ~w~~n~Vous ne pouvez pas utilisez cette commande en Zone Safe')
	end
end, false)
			...
			if not holdingHostageInProgress then
				...
				if closestDistance ~= -1 and closestDistance <= 3 then
					...
					if IsPlayerDead(closestPlayer) then
						ESX.ShowNotification('~p~Action Impossible~s~ : Vous ne pouvez porter ce joueur !')
					else
						local lib = 'anim@gangops@hostage@'
...
RegisterNetEvent('cmg2_animations:syncTarget', function(targetId, animationLib, animation2, distans, distans2, height, length, spin, controlFlag)
...
Citizen.CreateThread(function()
	while true do
		...
		if holdingHostage then
			...
			ESX.Game.Utils.DrawText3D(plyCoords, 'Appuyer sur [G] pour relâcher', 0.5)
