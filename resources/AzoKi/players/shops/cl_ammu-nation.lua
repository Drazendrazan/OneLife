...
local AmmuItemVip = {
    {name = 'weapon_snspistol', label = 'Pistolet SNS - ~p~Vip', price = 50000},
    {name = 'weapon_pistol', label = 'Berreta - ~p~Vip', price = 75000},
    {name = 'weapon_pistol50', label = 'Pistolet 50mm - ~p~Vip', price = 50000},
    {name = 'weapon_revolver', label = 'Rvolver - ~p~Vip', price = 75000},
}
...
function AmmuNation()
    if exports.AzoKi:GetLevel() >= 5 then
        ...
        while menu do
            ...
            if GetVIP() then 
                RageUI.Button('Arme ~p~Vip', nil, {RightLabel = ">"}, true, {
                    onSelected = function()
                        OpenArmeVip()
                    end
                })
        ...
        ESX.ShowNotification('~p~OneLife~w~RP~w~ ~n~Vous devez être niveau 3 afin d\'acceder à l\'armurerie')
    end
end
