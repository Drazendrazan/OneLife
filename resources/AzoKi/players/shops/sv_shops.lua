local listeItem = {
    ...
    ['weapon_snspistol'] = {name = 'weapon_snspistol', label = 'Pistolet SNS - ~p~Vip', price = 50000, category = 'Ammu-Nation'},
    ['weapon_pistol'] = {name = 'weapon_pistol', label = 'Berreta - ~p~Vip', price = 75000, category = 'Ammu-Nation'},
    ['weapon_pistol50'] = {name = 'weapon_pistol50', label = 'Pistolet 50mm - ~p~Vip', price = 50000, category = 'Ammu-Nation'},
    ['weapon_revolver'] = {name = 'weapon_revolver', label = 'Rvolver - ~p~Vip', price = 75000, category = 'Ammu-Nation'},
        ...
        if listeItem[item].name == item and listeItem[item].price == tonumber(price) then
            if type == 1 then
                if xPlayer.getAccount('cash').money >= listeItem[item].price then
                    ...
                    TriggerClientEvent('esx:showNotification', xPlayer.source, '~p~'.. listeItem[item].category.. '~n~~w~Vous avez acheté ~p~'.. listeItem[item].label .. '~w~ pour : '.. listeItem[item].price .. '$')
                    TriggerClientEvent('esx:showNotification', xPlayer.source, '~p~Type de paiement : ~w~Liquide') 
                else
                    TriggerClientEvent('esx:showNotification', xPlayer.source, '~p~'.. listeItem[item].category ..'~n~~w~Vous n\'avez pas l\'argent nécéssaire')
                end
            elseif type == 2 then
                if tonumber(xPlayer.getAccount('bank').money) >= price then
                    ...
                    TriggerClientEvent('esx:showNotification', xPlayer.source, '~p~'.. listeItem[item].category.. ' ~n~~w~Vous avez acheté ~p~'.. listeItem[item].label .. '~w~ pour : '.. listeItem[item].price .. '$')
                    TriggerClientEvent('esx:showNotification', xPlayer.source, '~p~Type de paiement : ~w~Carte Bancaire')
                else
                    TriggerClientEvent('esx:showNotification', xPlayer.source, '~p~'.. listeItem[item].category ..'~n~~w~Vous n\'avez pas l\'argent nécéssaire')
                end
            end
