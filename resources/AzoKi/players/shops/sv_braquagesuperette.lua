...
AddEventHandler('esx_holdup:robberyStarted', function(currentStore)
	...
	if Stores[currentStore] then
		...
		if (os.time() - store.lastRobbed) < cfg_braquage.TimerBeforeNewRob and store.lastRobbed ~= 0 then
			TriggerClientEvent('esx:showNotification', _source, _U('recently_robbed', cfg_braquage.TimerBeforeNewRob - (os.time() - store.lastRobbed)))
			return
		end
		...
		if not rob then
			if cops >= cfg_braquage.PoliceNumberRequired then
				rob = true
				...
				SetTimeout(store.secondsRemaining * 1000, function()
					if robbers[_source] then
						...
						if xPlayer then
							...
							if cfg_braquage.GiveBlackMoney then
								xPlayer.addAccountMoney('dirtycash', store.reward)
							else
				...
				TriggerClientEvent('esx:showNotification', _source, _U('min_police', cfg_braquage.PoliceNumberRequired))
			end
		else
