...
local function commencercarwash()
	...
	Wait(1000)
	fincarwashprops()
	--ShowAboveRadarMessage("~p~CarWash\n~s~Vous avez lavé votre ~p~"..GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(pVeh))).."~s~.")
		...
		for _,v in pairs(poscar) do
			if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
				...
				if Vdist2(GetEntityCoords(PlayerPedId(), false), v) < 7 then
					attente = 1
					DrawText3D(v.x, v.y, v.z+0.2, "Appuyez sur ~p~E ~w~pour ~p~laver votre véhicule~w~.", 9)
...
function openWarcash()
	...
	while menu do
		...
		RageUI.IsVisible(menu, function()
            RageUI.Button('Néttoyer sa Voiture ( 150$ )', nil, {}, true, {onSelected = function()
                TriggerServerEvent('framework:carwash')
            end})
        end, function()
        end)
...
RegisterNetEvent('framework:cleanvehicle')
AddEventHandler('framework:cleanvehicle', function()
