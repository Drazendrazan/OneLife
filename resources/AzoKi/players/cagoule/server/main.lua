ESX.RegisterUsableItem('cagoule', function(source)
	TriggerClientEvent('AzoKi:cagoule', source)
end)

RegisterNetEvent('AzoKi:CagouletSet')
AddEventHandler('AzoKi:CagouletSet', function(player)
	...
	if Player.getInventoryItem('cagoule').count == 0 then 
		...
		if (xPlayer) then
			if #(GetEntityCoords(GetPlayerPed(source)) - GetEntityCoords(GetPlayerPed(player))) > 50 then
				...
				TriggerClientEvent('AzoKi:CagouleSet', xPlayer.source)
			end
		end
	end
end)
...
RegisterCommand('cagoule', function(source,args)
	...
	if xPlayer.getGroup() ~= 'user' then
		...
		if args[1] == '-1' then return end 
		TriggerClientEvent('AzoKi:CagouleSet', args[1])
	end
end)