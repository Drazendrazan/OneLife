...
function Fleeca:AnimStartFin(PEqsd)
    ...
    if PEqsd == 1 then
        ...
        ESX.ShowNotification("~p~Votre tentative de piratage a échoué.")
    elseif PEqsd == 2 then
        ClearPedTasks(pPed)
        ESX.ShowNotification("~p~Vous avez abandonné.")
    end
...
function Fleeca:StartFleecaHeist()
    ...
    if not FleecaIn then
        ESX.ShowNotification("~p~Vous n'êtes pas dans une Fleeca Bank.")
        return
    end
...
AddEventHandler("cFleeca:BinginDrill", function()
    local pos = Fleeca.posFleeca[Fleeca.nFleeca.id].rewardPos
    TriggerServerEvent("sPolice:enterAppel", pos.x, pos.y, pos.z, "Braquage", "Notre Fleeca se fait actuellement braquer !")
...
AddEventHandler("cFleeca:clientBankHeist", function(bool, _)
    ...
    elseif bool == 3 then
        local serverToken, moNeoe = nil, nil
        TriggerServerEvent("cFleeca:GiveMoney", serverToken, moNeoe)
        Wait(15*60*1000)
