...
ESX.RegisterServerCallback('GangsBuilder:addArmoryWeapon', function(source, cb, weaponName, weaponAmmo)
	...
	if xPlayer.hasWeapon(weaponName) then
		TriggerEvent('esx_datastore:getSharedDataStore', 'society_'..xPlayer.job2.name, function(store)
			...
			--else
				--xPlayer.showNotification('~p~OneLife ~w~~n~Vous ne pouvez pas déposer les armes permanentes')
				--cb()
			--end
		end)
	else
...
RegisterNetEvent('GangsBuilder:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_' .. xPlayer.job2.name, function(inventory)
		...
		if sourceItem.count >= count and count > 0 then
			...
			xPlayer.showNotification('~p~OneLife ~w~~n~Vous avez déposer ~y~x'..count.. ' ~b~'.. ESX.GetItem(itemName).label)
			--local webhook = ''
			--SeaLogs(webhook, 'AzoKi', '\nLe joueur : __' .. GetPlayerName(xPlayer.source) .. ' à déposer dans __\nNom du gang : '..xPlayer.job2.name.. '\nLabel du Gang : '..xPlayer.job2.label..'\nItem déposer :'.. itemName..'\nQuantités : '.. count, 56108)
		else
			xPlayer.showNotification('~p~OneLife ~w~~n~Vous n\'avez pas la quantité nécéssaire pour déposer dans le coffre')
		end
	end)
end)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_' .. xPlayer.job2.name, function(inventory)
		...
		if inventoryItem.count >= count and inventoryItem.count > 0 then
			if xPlayer.canCarryItem(itemName, count) then
				...
				xPlayer.showNotification('~p~OneLife ~w~~n~Vous avez retiré ~y~x'.. count.. ' ~b~' ..inventoryItem.label)
			else
				xPlayer.showNotification('~p~OneLife ~w~~n~Vous ne pouvez pas prendre autant d\'objets sur vous')
			end
		else
			xPlayer.showNotification('~p~OneLife ~w~~n~Le coffre ne contient pas la quantité que vous souhaitez prendre')
		end
	end)
end)
...
RegisterNetEvent('Gangsbuilder:BuyWeapon', function(weapon, label)
	...
	if xPlayer.getAccount('dirtycash').money >= SecurityWeaponGangsBuilder[weapon].price then
        if not (HaveWeaponInLoadout(xPlayer, SecurityWeaponGangsBuilder[weapon].hash)) then
			...
			xPlayer.addWeapon(SecurityWeaponGangsBuilder[weapon].hash, 500)
			xPlayer.showNotification('~p~OneLife~w~ Vous avez acheté ~b~'..label)
		else
			xPlayer.showNotification('~p~OneLife ~w~~n~Vous avez déjà cette arme sur vous !')
		end
	else
		xPlayer.showNotification('~p~OneLife ~w~~n~Vous n\'avez pas l\'argent nécéssaire.')
	end
end)

RegisterCommand('gangsbuilder', function(source,args)
	local xPlayer = ESX.GetPlayerFromId(source)
