...
local mainMenu = RageUI.CreateMenu("", "~p~Animation")

local walk = RageUI.CreateSubMenu(mainMenu, "", "~p~Démarches")
local sexe = RageUI.CreateSubMenu(mainMenu, " ", "~p~+21")
local danse = RageUI.CreateSubMenu(mainMenu, " ", "~p~Danses")
local poses = RageUI.CreateSubMenu(mainMenu,"","~p~Poses")
local sit = RageUI.CreateSubMenu(mainMenu,"","~p~S'asseoir/Tomber")
local works = RageUI.CreateSubMenu(mainMenu,"","~p~Métier")
local gang = RageUI.CreateSubMenu(mainMenu,"","~p~Gang")
local sport = RageUI.CreateSubMenu(mainMenu,"","~p~Sport")
local frap = RageUI.CreateSubMenu(mainMenu,'', "~p~ Frapper ") 
local actions = RageUI.CreateSubMenu(mainMenu,'', "~p~ Actions ") 
local salue = RageUI.CreateSubMenu(mainMenu,"", "~p~Salue")
local props = RageUI.CreateSubMenu(mainMenu , "", "~p~Emote disponible")
...
function renderanim(label,desc, value, anim,loop)
    RageUI.Button(label, "/e ~p~"..desc, {RightLabel = ""}, true, {
        onSelected = function()
            DestroyAllProps()
            Sotek.handsUp = false
...
function renderanimoneprops(label,desc, value, anim,loop, name,bone,PropPlacement)
    RageUI.Button(label, "/e ~p~"..desc, {RightLabel = "(props)"}, true, {
        onSelected = function()
            DestroyAllProps()
            Sotek.handsUp = false
