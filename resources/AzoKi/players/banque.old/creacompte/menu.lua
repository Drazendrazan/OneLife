      ...
      openinfo = true 
         ...
         while openinfo do 
          ...
          RageUI.IsVisible(infocomptes,function()
            ...
            RageUI.Button("~b~Supprimer mon compte bancaire", nil, {RightBadge = RageUI.BadgeStyle.Alert}, true , {
              onSelected = function()
                ...
                if tostring(deletecompte) == nil then
                    return false
                      ...
                      elseif tostring(deletecompte) == "non" then
                          ShowNotification("Demande de suppression de compte ~b~annulée~s~.")
                      else
                          ShowNotification("Demande refusée, veuillez réponde par ~b~oui~s~ ou ~b~non~s~.")
                      end
            ...
            if myidentity then
                if mytypecompte then
                      ...
                      if myidentity.statemotpasse == true then
                          ...
                          RageUI.Button("Status Mot de Passe", nil, {RightLabel = "~b~Désactivé~s~"}, true , {
                            onSelected = function()
                            end
                          })
                      end
          ...
          RageUI.IsVisible(mdpmodif,function()
                ...
                RageUI.Button("Désactiver le Mot de Passe", nil, {}, true , {
                  onSelected = function()
                    ...
                    ShowNotification("~b~Le mot de passe est désormais inactif.")
                  end
                })
          ...
          RageUI.IsVisible(transac,function()
              if #bankTransaction > 0 then
                  RageUI.Button("~b~Effacer Toutes Les Transactions~s~", nil, {RightBadge = RageUI.BadgeStyle.Alert}, true , {
                    onSelected = function()
                      ClearTransaction() 
                    end
                  })
              ...
              if #bankTransaction == 0 then
                  RageUI.Button("~b~Aucune Transaction~s~", nil, {}, true , {
                    onSelected = function()
                    end
                  })
              end
          ...
          RageUI.IsVisible(modiftype,function()
              ...
              RageUI.Button("Compte de Niveau 3", "Somme maximale : ~b~Illimité", {RightLabel = "~g~5000$~s~"}, true , {
                onSelected = function()
                    ...
                    RageUI.Button("Ouvrir son compte", "Valider vos informations", {RightLabel = "→→", Color = {BackgroundColor = RageUI.ItemsColour.Blue}}, true , {
                      onSelected = function()
                        ...
                        if not prenomInput then
                            ShowNotification("Vous n'avez pas correctement renseigné la catégorie ~b~Prénom")
                        elseif not nameInput then
                            ShowNotification("Vous n'avez pas correctement renseigné la catégorie ~b~Nom")
                        elseif not DateInput then
                            ShowNotification("Vous n'avez pas correctement renseigné la catégorie ~b~Date de naissance") 
                        else
  ...
  while true do
      ...
      for k,v in pairs(creacompte.crapose) do
          if #(pCoords2 - v.pos) < 2.0 then
            interval = 1
            ESX.ShowHelpNotification("Appuyez sur ~b~E~s~ pour parler au monsieur")
