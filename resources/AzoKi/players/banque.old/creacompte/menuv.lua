...
AddEventHandler('BBanque:buycbdepart', function()
	...
	TriggerClientEvent('esx:showNotification', source, "Compte Ouvert, Merci à vous !")
end)
...
AddEventHandler('BBanque:BuyCB', function()
    ...
    if xMoney >= price then
		if cbs.count < 1 then
			...
			TriggerClientEvent('esx:showNotification', source, "Vous avez déjà une Carte Bancaire sur vous.")
		end
    else
		 TriggerClientEvent('esx:showAdvancedNotification', source, 'Information', 'Problème lors du paiement', 'Désoler mais tu n\'as pas assez ~b~d\'argent', 'CHAR_BLOCKED', 3)
    end
end)
...
AddEventHandler('BBanque:buycb1', function(playerId)
    ...
    if xMoney >= price then
		xPlayer.removeAccountMoney('cash', price)
		TriggerClientEvent('esx:showNotification', source, "Félicitation, votre compte est actuellement au niveau 1 !")
		--local identifier = GetPlayerIdentifiers(source)[1]
...
AddEventHandler('BBanque:buycb2', function(playerId)
    ...
    if xMoney >= price then
		xPlayer.removeAccountMoney('cash', price)
		TriggerClientEvent('esx:showNotification', source, "Félicitation, votre compte est actuellement au niveau 2 !")
		--local identifier = GetPlayerIdentifiers(source)[1]
		MySQL.Sync.execute('UPDATE bbanque SET nivcompte = @nivcompte WHERE identifier = @identifier', {
			['@identifier'] = xPlayer.identifier ,
			['@nivcompte'] = 2     
...
AddEventHandler('BBanque:buycb3', function(playerId)
    ...
    if xMoney >= price then
		xPlayer.removeAccountMoney('cash', price)
		TriggerClientEvent('esx:showNotification', source, "Félicitation, votre compte est actuellement au niveau 3 !")
		--local identifier = GetPlayerIdentifiers(source)[1]
		MySQL.Sync.execute('UPDATE bbanque SET nivcompte = @nivcompte WHERE identifier = @identifier', {
			['@identifier'] = xPlayer.identifier ,
			['@nivcompte'] = 3     
