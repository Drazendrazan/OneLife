	...
	ESX.TriggerServerCallback('Core:requestPlayerCars', function(gotkey)
		if gotkey then
			...
			if locked == 1 or locked == 0 then
				...
				ESX.ShowAdvancedNotification('~p~OneLife', '~y~Clés', "Vous avez ~b~fermé~p~ le véhicule.", 'CHAR_SEALIFE', 7)
			elseif locked == 2 then
				SetVehicleDoorsLocked(vehicle, 1)
				PlayVehicleDoorOpenSound(vehicle, 0)
				ESX.ShowAdvancedNotification('~p~OneLife', '~y~Clés', "Vous avez ~p~ouvert~p~ le véhicule.", 'CHAR_SEALIFE', 7)
			end
		else
			ESX.ShowAdvancedNotification('~p~OneLife', '~y~Clés', "~b~Vous n'avez pas les clés de ce véhicule.", 'CHAR_SEALIFE', 7)
		end
