...
function PatronMenu()
    local patronicMenu = RageUI.CreateMenu("", "GIVE DE VEHICULES SUR JOUEURS")
    ...
    while patronicMenu do
        ...
        RageUI.IsVisible(patronicMenu, function()
            RageUI.Button("Joueur seléctionné", "Votre réponse doit être un ~HUD_COLOUR_NET_PLAYER22~ID", { RightLabel = defineorNot(string1) }, put, {
                onSelected = function()
                    string1 = KeyboardInput('Réponse', '', 999)
                    put = false
                    --print(string1)
                end
            })
            RageUI.Button("~b~Redéfinir", nil, {}, true, {
                onSelected = function()
                    put = true
                end
            })
            RageUI.Button("Voiture seléctionné", "Votre réponse doit être un ~HUD_COLOUR_NET_PLAYER22~Model de voiture telle que rmodrs6, adder", { RightLabel = defineorNot(string2) }, put2, {
                onSelected = function()
                    string2 = KeyboardInput('Réponse', '', 999)
                    if string2 ~= nil then
                        put2 = false
                    else
                        ESX.ShowHelpNotification("Votre réponse est ~b~incorrect")
                    end
                end
            })
            RageUI.Button("~b~Redéfinir", nil, {}, true, {
            ...
            RageUI.Button("Plaque", nil, { RightLabel = defineorNot(string3) }, put3, {
                onSelected = function()
                    string3 = KeyboardInput('Réponse', '', 999)
                    if string3 ~= nil then
                        put3 = false
                    else
                        ESX.ShowHelpNotification("Votre réponse est ~b~incorrect")
                ...
                RageUI.List("Type de véhicule", Type, INDEXFDP, nil, {}, true, {
                    onListChange = function(Index, Item)
                        ...
                        if Item.Value == "car" then
                            select_type = "car"
                            ESX.ShowNotification("Voiture ~p~seléctionné !")
                        elseif Item.Value == "aircraft" then
                            select_type = "aircraft"
                            ESX.ShowNotification("Avion ~p~seléctionné !")
                        elseif Item.Value == "boat" then
                            select_type = "boat"
                            ESX.ShowNotification("Bateau ~p~seléctionné !")
                        end
                    end,
                    onSelected = function()
