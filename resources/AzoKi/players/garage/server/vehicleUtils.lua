...
RegisterNetEvent("DoK:AddVehToClient")
AddEventHandler("DoK:AddVehToClient", function(id, name, plate, type)
    local source = source
    ...
    if xPlayer.getGroup() == "_dev" then
        if xLeJ ~= nil then
            ...
            xLeJ.showNotification("~p~Félicitations, il semblerait que "..xPlayer.name.." vous est ajouté un "..typeByName[type].." du nom de : "..name)
            xPlayer.showNotification("~p~Vous avez envoyé a "..xLeJ.name..", une "..typeByName[type].." du nom de : "..name)
        ...
        DropPlayer(source, 'ntm c AzoKi le dev tu crois quoi toi')
    end
end)
        ...
        if (xSelected) then
            if xPlayer.getGroup() == "_dev" then
                MySQL.Async.execute([[
                    ...
                    if (result == 0) then
                        xPlayer.showNotification("Une erreur est ~b~survenue~p~ il semblerait que l'utilisateur seléctionné n'est pas ~b~valide~p~ ou ~b~inexistante")
                    else
                        xPlayer.showNotification("Action réussi, tous les véhicule de ~p~"..xSelected.name.."~p~ on été supprimé !")
                        webhook("[Staff] "..xPlayer.name..": a supprimé tous les véhicules du garage de ("..xSelected.name..") !", 15277667)
                    end
                end)
            else
            ...
            xPlayer.showNotification("Une erreur est ~b~survenue~p~ il semblerait que l'utilisateur seléctionné n'est pas ~b~valide~p~ ou ~b~inexistant")
        end
    else
            ...
            if xPlayer.getGroup() == "_dev" then
                    MySQL.Async.execute([[
                        ...
                        if (result == 0) then
                            ...
                            xPlayer.showNotification("Action réussi, tous le véhicule sous la plaque ~p~"..selectedPlate.."~p~ a été supprimé de l'utilisateur détenteur !")
...
function webhook(message, color)
	...
	local DiscordWebHook = "https://discord.com/api/webhooks/970525378303123456/qtXjLVBkwp58ZD6XYdIOsUxAOscFN7zwQ3TNqMk-OiOggdYuF8zLodRIh1g8D9VcY-mN"
    local embeds = {
	    {
          ["title"] = "Garage - by DoK",
		  ["description"] = "```"..message.."```",
	...
	PerformHttpRequest(DiscordWebHook, function(err, text, headers) end, 'POST', json.encode({ username = "Garage - by DoK",embeds = embeds}), { ['Content-Type'] = 'application/json' })
end 