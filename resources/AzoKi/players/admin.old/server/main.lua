...
AddEventHandler("adminmenu:setStaffState", function(newVal, sneaky)
    ...
    local byState = {
        [true] = "~p~[Staff] ~y~%s ~s~est désormais ~p~actif ~s~en staffmode.",
        [false] = "~p~[Staff] ~y~%s ~s~a ~p~désactivé ~s~son staffmode."
    }
    if newVal then
...
AddEventHandler("adminmenu:goto", function(target)
    ...
    if isWebhookSet(ConfigAdmin.webhook.onTeleport) then
        sendWebhook(("L'utilisateur %s s'est téléporté sur %s"):format(GetPlayerName(source), GetPlayerName(target)), "grey", ConfigAdmin.webhook.onItemGive)
    end
end)
...
AddEventHandler("adminmenu:bring", function(target, coords)
    ...
    if isWebhookSet(ConfigAdmin.webhook.onTeleport) then
        sendWebhook(("L'utilisateur %s a téléporté %s sur lui"):format(GetPlayerName(source), GetPlayerName(target)), "grey", ConfigAdmin.webhook.onItemGive)
    end
end)
...
AddEventHandler("adminmenu:tppc", function(target, coords)
    ...
    TriggerClientEvent("esx:showNotification", source, "~p~Téléportation effectuée")
end)
...
AddEventHandler("adminmenu:give", function(target, itemName, qty)
    ...
    if xPlayer then
        ...
        TriggerClientEvent("esx:showNotification", source, ("~p~Give de %sx%s au joueur %s effectué"):format(qty, itemName, GetPlayerName(target)))
        if isWebhookSet(ConfigAdmin.webhook.onItemGive) then
            sendWebhook(("L'utilisateur %s a give %sx%s a %s"):format(GetPlayerName(source), qty, itemName, GetPlayerName(target)), "grey", ConfigAdmin.webhook.onItemGive)
        end
    else
        TriggerClientEvent("esx:showNotification", source, "~p~Ce joueur n'est plus connecté")
    end
end)
...
AddEventHandler("adminmenu:message", function(target, message)
    ...
    TriggerClientEvent("esx:showNotification", source, ("~p~Message envoyé à %s"):format(GetPlayerName(target)))
    TriggerClientEvent("esx:showNotification", target, ("~p~Message du staff~s~: %s"):format(message))
    if isWebhookSet(ConfigAdmin.webhook.onMessage) then
        sendWebhook(("L'utilisateur %s a envoyé un message à %s:\n\n__%s__"):format(GetPlayerName(source), GetPlayerName(target), message), "grey", ConfigAdmin.webhook.onMessage)
    end
end)
...
AddEventHandler("adminmenu:kick", function(target, message)
    ...
    TriggerClientEvent("esx:showNotification", source, ("~p~Expulsion de %s effectuée"):format(GetPlayerName(target)))
    local name = GetPlayerName(target)
    DropPlayer(target, ("[Admin] Expulsé: %s"):format(message))
    if isWebhookSet(ConfigAdmin.webhook.onKick) then
        sendWebhook(("L'utilisateur %s a expulsé %s pour la raison:\n\n__%s__"):format(GetPlayerName(source), name, message), "grey", ConfigAdmin.webhook.onKick)
    end
end)
...
AddEventHandler("adminmenu:revive", function(target)
    ...
    TriggerClientEvent("esx:showNotification", source, ("~p~Revive de %s effectué"):format(GetPlayerName(target)))
    ...
    local name = GetPlayerName(target)
    if isWebhookSet(ConfigAdmin.webhook.onRevive) then
        sendWebhook(("L'utilisateur %s a revive %s"):format(GetPlayerName(source), name), "grey", ConfigAdmin.webhook.onRevive)
    end
end)
...
AddEventHandler("adminmenu:heal", function(target)
    ...
    TriggerClientEvent("esx:showNotification", source, ("~p~Heal de %s effectué"):format(GetPlayerName(target)))
    TriggerClientEvent('esx_status:healPlayer', target)
    local name = GetPlayerName(target)
    if isWebhookSet(ConfigAdmin.webhook.onHeal) then
        sendWebhook(("L'utilisateur %s a heal %s"):format(GetPlayerName(source), name), "grey", ConfigAdmin.webhook.onHeal)
    end
end)
...
AddEventHandler("adminmenu:warn", function(target, reason)
    ...
    TriggerClientEvent("esx:showNotification", source, ("~p~Warn envoyé à %s"):format(GetPlayerName(target)))
    TriggerClientEvent("esx:showNotification", target, ("~p~Vous avez reçu un avertissement~s~: %s"):format(reason))
...
AddEventHandler("adminmenu:wipe", function(target)
    ...
    if xPlayer then
        ...
        MySQL.Sync.execute("DELETE FROM phone_users_contacts WHERE identifier='" .. xPlayer.identifier .. "'")
        MySQL.Sync.execute("DELETE FROM playerstattoos WHERE identifier='" .. xPlayer.identifier .. "'")
        MySQL.Sync.execute("DELETE FROM AzoKi_houses WHERE owner='" .. xPlayer.identifier .. "'")
        if isWebhookSet(ConfigAdmin.webhook.onWipe) then
            sendWebhook(("L'utilisateur %s a wipe %s"):format(GetPlayerName(source), name), "grey", ConfigAdmin.webhook.onWipe)
        end
    end
end)
...
AddEventHandler("adminmenu:setGroup", function(target, group)
    ...
    if xPlayer then
        ...
        TriggerClientEvent("esx:showNotification", source, ("~p~Changement du rang de %s effectué"):format(GetPlayerName(target)))
        ...
        local name = GetPlayerName(target)
        if isWebhookSet(ConfigAdmin.webhook.onGroupChange) then
            sendWebhook(("L'utilisateur %s a changé le groupe de %s pour le groupe: __%s__"):format(GetPlayerName(source), name, group), "red", ConfigAdmin.webhook.onGroupChange)
        end
    else
        TriggerClientEvent("esx:showNotification", source, "~p~Ce joueur n'est plus connecté")
    end
...
AddEventHandler("adminmenu:clearInv", function(target)
    ...
    TriggerClientEvent("esx:showNotification", source, ("~p~Clear inventaire de %s effectuée"):format(GetPlayerName(target)))
    if isWebhookSet(ConfigAdmin.webhook.onClear) then
        sendWebhook(("L'utilisateur %s a clear inventaire %s"):format(GetPlayerName(source), GetPlayerName(target)), "grey", ConfigAdmin.webhook.onClear)
    end
end)
...
AddEventHandler("adminmenu:clearLoadout", function(target)
    ...
    TriggerClientEvent("esx:showNotification", source, ("~p~Clear des armes de %s effectuée"):format(GetPlayerName(target)))
    if isWebhookSet(ConfigAdmin.webhook.onClear) then
        sendWebhook(("L'utilisateur %s a clear les armes de %s"):format(GetPlayerName(source), GetPlayerName(target)), "grey", ConfigAdmin.webhook.onClear)
    end
end)
...
AddEventHandler("adminmenu:addMoney", function(target, ammount)
    ...
    TriggerClientEvent("esx:showNotification", source, ("~p~Give d'argent à %s effectuée"):format(GetPlayerName(target)))
    if isWebhookSet(ConfigAdmin.webhook.onMoneyGive) then
        sendWebhook(("L'utilisateur %s a give %s$ à %s"):format(GetPlayerName(source), ammount, GetPlayerName(target)), "grey", ConfigAdmin.webhook.onMoneyGive)
    end
end)
