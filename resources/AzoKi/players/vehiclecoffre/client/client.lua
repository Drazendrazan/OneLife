	...
	if vehicle ~= nil and IsPedOnFoot(plyPed) then
		if DoesVehicleHaveDoor(vehicle, 5) or VehicleCoffre.NoTrunkVehicles[GetEntityModel(vehicle)] ~= nil then
			if GetVehicleDoorLockStatus(vehicle) == 1 then
				...
				ESX.ShowNotification('Ce coffre est ~p~fermé')
			end
		else
			ESX.ShowNotification('Ce ~p~véhicule~w~ ne possède pas de coffre.')
		end

		ESX.ShowNotification('Pas de ~p~coffre~w~ à proximité')
	end
end, false)
	...
	while inventaireplayer do
        ...
        RageUI.IsVisible(inventaireplayer, function()
            ...
            for k,v in pairs(data.inventory) do
                RageUI.Button(v.label, nil, { RightLabel = "Quantité(e)s : ~p~"..v.count }, true, {
                    onSelected = function()
                        quantity = KeyboardInput('Combien voulez vous déposer ?', '', 30)
						AzoKiClientUtils.toServer('esx_truck_inventory:putItem', v.name, 'item_standard', tonumber(quantity), GetVehicleNumberPlateText(vehicle))
                        RageUI.CloseAll()
                    end
                })
            end
            ...
            for k,v in pairs(data.weapons) do
                RageUI.Button(v.label, nil, { RightLabel = "Munitions : ~p~"..v.ammo }, true, {
                    onSelected = function()
                        AzoKiClientUtils.toServer('esx_truck_inventory:putItem', v.name, 'item_weapon', v.ammo, GetVehicleNumberPlateText(vehicle))
                        RageUI.CloseAll()
                    end
                })
            end
        ...
        RageUI.IsVisible(inventaireplayer, function()
            ...
            for k,v in pairs(data2.items) do 
                if v.count ~= 0 then
                    RageUI.Button(v.label, nil, { RightLabel = "Quantité(e)s : ~p~"..v.count }, true, {
                        onSelected = function()
                            quantity = KeyboardInput('Combien voulez vous prendre ?', '', 30)
                            AzoKiClientUtils.toServer('esx_truck_inventory:getItem', v.name, 'item_standard', tonumber(quantity), GetVehicleNumberPlateText(vehicle))
                            RageUI.CloseAll()
                        end
                    })
                end
            ...
            for k,v in pairs(data2.weapons) do
                RageUI.Button(ESX.GetWeaponLabel(v.name), nil, { RightLabel = "Munitions : ~p~"..v.ammo }, true, {
                    onSelected = function()
						AzoKiClientUtils.toServer('esx_truck_inventory:getItem', v.name, 'item_weapon', v.ammo, GetVehicleNumberPlateText(vehicle))
                        RageUI.CloseAll()
                    end
                })
            end
