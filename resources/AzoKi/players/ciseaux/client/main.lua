RegisterNetEvent('altix:useciseaux', function()
    if not GetSafeZone() then
        ...
        if closestPlayer ~= -1 and closestDistance <= 3.0 then
            ...
            ESX.ShowNotification('~p~Impossible~s~~n~personne a proximité de vous.')
        end
    else
        ESX.ShowNotification('~p~OneLife ~w~~n~Vous ne pouvez pas couper les cheveux de quelqu\'un en Zone Safe')
    end
end)
