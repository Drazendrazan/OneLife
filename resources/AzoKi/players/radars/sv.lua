...
Citizen.CreateThread(function()
	...
	ESX.RegisterUsableItem('waze', function(source, xPlayer)
		...
		xPlayer.showNotification('~p~Votre waze est actif pour 1h')
...
function ResetRadars()
	...
	for _, xPlayerPolice in pairs(ESX.GetPlayersWithJob('police')) do
		xPlayerPolice.showNotification("~p~Réparation de tous les radars par un technicien...")
	end
end
...
AddEventHandler('xr_speedradar:destroyedRadar', function(RadarId)
	...
	for _, xPlayerPolice in pairs(ESX.GetPlayersWithJob('police')) do
		xPlayerPolice.showNotification('~r~Radar détruit~w~ près de la ~p~'.. Radars.Radars[RadarId].Name.."~w~.")
	end
end)
...
AddEventHandler('xr_speedradar:flashed', function(plate, vitesse, model, radarId)
	...
	for _, xPlayerPolice in pairs(ESX.GetPlayersWithJob('police')) do
		xPlayerPolice.showNotification('~p~'.. model ..'~w~ immatriculé ~p~' .. plate .. '~w~ flashé à ~p~'.. vitesse .. 'KM/H~w~ près de la ~p~'.. Radars.Radars[radarId].Name.."~w~.")
	end

	if xPlayer.getAccount('bank').money >= Radars.BillPrice then
		xPlayer.showNotification('~p~Vous avez été flashé ! ~r~Vous avez payé ' .. Radars.BillPrice .. '$ depuis votre compte bancaire ...')
		xPlayer.removeAccountMoney('bank', Radars.BillPrice)
	else
		xPlayer.showNotification('~p~Vous avez été flashé ! ~r~Vous avez reçu une amende de ' .. Radars.BillPrice .. '$ ...')
