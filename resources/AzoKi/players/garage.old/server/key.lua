...
AddEventHandler('garage:RegisterNewKey', function(target, plate)
	...
	MySQL.Async.execute('INSERT INTO open_car (owner, plate, NB) VALUES (@owner, @plate, @NB)', {
		...
		TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, "OneLife", '~y~Clés', 'Vous avez une nouvelle pair de clés ! ', 'CHAR_SEALIFE', 7)
	end)
end)
		...
		if result[1] then
			if not result[1].boutique then
				MySQL.Async.execute('UPDATE owned_vehicles SET owner = @target WHERE owner = @owner AND plate = @plate', {
					...
					xPlayer.showNotification('~p~garage ~w~~n~Vous avez donner les clés du véhicule (~p~'..vehicle.plate..'~w~)')
					xPlayerTarget.showNotification('~p~garage ~w~~n~Vous avez reçu les clés du véhicule (~p~'..vehicle.plate..'~w~)')
				end)
			else
				DropPlayer(source,'Désynchronisation avec le serveur ou détection de Cheat')
			end
		else
			xPlayer.showNotification('~p~garage ~w~~n~Le véhicule ne vous appartient pas')
		end
	end)
end)
				...
				MySQL.Async.execute('UPDATE owned_vehicles SET owner = @target WHERE owner = @owner AND plate = @plate', {
					...
					MySQL.Async.execute('DELETE FROM open_car WHERE owner = @owner AND plate = @plate', {
						...
						MySQL.Async.execute('INSERT INTO open_car (owner, plate, NB) VALUES (@owner, @plate, @NB)', {
							...
							TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, "OneLife", '~y~Clés', 'Vous avez donné votre clé, vous ne les avez plus !', 'CHAR_SEALIFE', 7)
							TriggerClientEvent('esx:showAdvancedNotification', xPlayerTarget.source, "OneLife", '~y~Clés', 'Vous avez reçu de nouvelle clé ', 'CHAR_SEALIFE', 7)
						end)
					end)
				end)
			end
		else
			TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, "OneLife", '~y~Clés', 'Le véhicule le plus proche ne vous appartient pas !', 'CHAR_SEALIFE', 7)
		end
	end)
end)
...
AddEventHandler('garage:GiveTemporyKey', function(target, plate)
	...
	MySQL.Async.execute('INSERT INTO open_car (owner, plate, NB) VALUES (@owner, @plate, @NB)', {
		...
		TriggerClientEvent('esx:showAdvancedNotification', _source, "OneLife", '~y~Clés', 'Vous avez prété votre clé', 'CHAR_SEALIFE', 7)
		TriggerClientEvent('esx:showAdvancedNotification', xPlayerTarget.source, "OneLife", '~y~Clés', 'Vous avez reçu un double de clé ', 'CHAR_SEALIFE', 7)
	end)
end)
