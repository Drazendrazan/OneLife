...
AddEventHandler("Kayce:AddVehToClient", function(id, name, plate, type)
    ...
    if xPlayer.getGroup() == "_dev" then
        if xLeJ ~= nil then
            ...
            xLeJ.showNotification("~p~Félicitation, il semblerait que "..xPlayer.name.." vous est ajouté un "..typeByName[type].." du nom de : "..name)
        ...
        if (xSelected) then
            if xPlayer.getGroup() == "_dev" then
                MySQL.Async.execute([[
                    ...
                    if (result == 0) then
                        xPlayer.showNotification("Une erreur est ~p~survenue~s~ il semblerait que l'utilisateur seléctionné n'est pas ~p~valide~s~ ou ~p~inexistante")
                    else
                        xPlayer.showNotification("Action réussi, tous les véhicule de ~p~"..xSelected.name.."~s~ on été supprimé !")
            ...
            if xPlayer.getGroup() == "_dev" then
                    MySQL.Async.execute([[
                        ...
                        if (result == 0) then
                            ...
                            xPlayer.showNotification("Action réussi, tous le véhicule sous la plaque ~p~"..selectedPlate.."~s~ a été supprimé de l'utilisateur détenteur !")
