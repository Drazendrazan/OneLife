            ...
            if tostring(TypeGarage) == 'car' then
                ...
                RageUI.Separator('Vos véhicule(s).')
                    for k,v in pairs(localCarsOwned) do
                        if v.type == 'car' then
                            if v.label ~= nil then 
                                RageUI.Button('['..GetLabelText(GetDisplayNameFromVehicleModel(v.vehicle.model))..'] - '..v.label, (v.state == true and "Informations sur votre véhicule\n\nPlaque : "..v.plate.."\n\nModel : "..getByName(GetLabelText(GetDisplayNameFromVehicleModel(v.vehicle.model))).."" or "~p~Se véhicule se trouve à la Fourière !"), {RightBadge = RageUI.BadgeStyle.Tick}, v.state, {
                                    onActive = function()
                                ...
                                RageUI.Button(GetLabelText(GetDisplayNameFromVehicleModel(v.vehicle.model)), (v.state == true and "Informations sur votre véhicule\n\nPlaque : "..v.plate.."\n\nModel : "..getByName(GetLabelText(GetDisplayNameFromVehicleModel(v.vehicle.model))).."" or "~p~Se véhicule se trouve à la Fourière !"), {}, v.state, {
                                    onActive = function()
                                        if (IsControlJustPressed(1, 49)) then
                                            local input = KeyboardInput('Renommer votre véhicule', v.label, 20);
                                            if (input ~= nil) and (#input < 0) and (#input > 20) then
                        ...
                        if v.type == 'car' and not v.state then
                            if v.label ~= nil then 
                                RageUI.Button('['..GetLabelText(GetDisplayNameFromVehicleModel(v.vehicle.model))..'] - '..v.label, "Informations sur votre véhicule\n\nPlaque : "..v.plate.."\n\nModel : "..getByName(GetLabelText(GetDisplayNameFromVehicleModel(v.vehicle.model))).."", {}, not v.state, {
                                        ...
                                        if (IsControlJustPressed(1, 246)) then
                                            ESX.TriggerServerCallback('garage:storeVehicleFourriere', function(valid)
                                                if valid then
                                                    ...
                                                    ESX.ShowNotification('Vous avez payé ~p~500$~s~ la fourrière de '..v.label or "Votre Voiture")
                                                    RageUI.CloseAll()
                                                    Wait(150)
                                                    openMenuGarage(SpawnPoint, TypeGarage, Heading)
                                                else
                                    ...
                                    onSelected = function()
                                        Visual.Subtitle("~p~Facture :~s~ Appuyer sur ~p~Y~s~ pour payer la fourière de ~p~"..v.label or "Votre Voiture", 2000)
                                    end
                                })
                            else
        ...
        RageUI.IsVisible(menu, function()
            RageUI.Button('Sortir le véhicule', nil, {}, true, {
                onSelected = function()
                    ...
                    RageUI.CloseAll()
                    ESX.ShowNotification('Vous avez sorti ~p~'..vehicle.label.."~s~ du garage de "..zoneLabel)
                end
            })
            if not vehicle.boutique then
                RageUI.Button('Donner le véhicule', 'Permet de donner le véhicule au joueurs le plus proche de vous', {}, true, {
                    ...
                    onSelected = function()
                        local player, closestplayer = ESX.Game.GetClosestPlayer()
                        if player == -1 or closestplayer > 3.0 then
                            ESX.ShowNotification('~p~Aucun joueurs au alentours.')
                            return
                        else
            ...
            if not entreprise then
                RageUI.Button('Attribuer le véhicule à son entreprise', 'Permet d\'attribuer le véhicule à votre entreprise\n\n~p~Avertissement ~s~: Vous ne pourrez en aucun cas le récupérer !', {}, true, {
                    onSelected = function()
                        RageUI.CloseAll()
                        if not vehicle.boutique then
                            TriggerServerEvent('garage:AttribuerVehicule', 'job', vehicle)
                        else
                            ESX.ShowNotification('~p~Avertissement~s~ : Vous ne pouvez pas attribuer un véhicule de la boutique a votre Entreprise.')
                        end
                    end
                })
            end
                ...
                RageUI.Button('Attribuer le véhicule à son Gang/Organisation', 'Permet d\'attribuer le véhicule à votre Gang/Organisation\n\n~p~Avertissement ~s~: Vous ne pourrez en aucun cas le récupérer !', {}, true, {
                    onSelected = function()
                        ...
                        if not vehicle.boutique then
                            ...
                            ESX.ShowNotification('~p~Avertissement~s~ : Vous ne pouvez pas attribuer un véhicule de la boutique a votre Gang/Organisation.')
                        end
                    end
                })
            end
    ...
    while modify do
        ...
        RageUI.IsVisible(modify, function()
            for _, data in pairs(Garage) do
                RageUI.Separator("↓ Garage ~p~n°"..data.id.."~s~ ↓")
                ...
                RageUI.Button("Marker Garage", "X : "..data.position.x.."\nY : "..data.position.y.."\nZ : "..data.position.z, {}, true, {
                    onActive = function()
                        roo = true
                        if nice then
                            Visual.Subtitle("Appuyer sur ~p~E~s~ pour changer le point du garage !", 1)
                        else
                            Visual.Subtitle("Appuyer sur ~p~ENTRÉE~s~ pour vous téléporter au point de spawn !", 1)
                        end
                ...
                RageUI.Button("Point de Spawn", "X : "..data.SpawnPoint.x.."\nY : "..data.SpawnPoint.y.."\nZ : "..data.SpawnPoint.z, {}, true, {
                    onActive = function()
                        roo = true
                        if nice then
                            Visual.Subtitle("Appuyer sur ~p~E~s~ pour changer le point de spawn !", 1)
                        else
                            Visual.Subtitle("Appuyer sur ~p~ENTRÉE~s~ pour vous téléporter au point de spawn !", 1)
                        end
                ...
                RageUI.Button("Allignement du spawn", "Heading : "..data.Heading, {}, true, {
                    onActive = function()
                        roo = true
                        if nice then
                            Visual.Subtitle("Appuyer sur ~p~E~s~ pour changer l'alignement !", 1)
                        else
                            Visual.Subtitle("Appuyer sur ~p~ENTRÉE~s~ pour vous alligner au même niveau !", 1)
                        end
                ...
                RageUI.Button("Ranger le véhicule", "X : "..data.DeletePoint.x.."\nY : "..data.DeletePoint.y.."\nZ : "..data.DeletePoint.z, {}, true, {
                    onActive = function()
                        roo = true
                        if nice then
                            Visual.Subtitle("Appuyer sur ~p~E~s~ pour changer le point de suppression !", 1)
                        else
                            Visual.Subtitle("Appuyer sur ~p~ENTRÉE~s~ pour vous téléporter au point de suppression !", 1)
                        end
    ...
    while true do 
        ...
        for k,v in pairs(Garage) do
            ...
            if DistanceGarage < 3 then
                ESX.ShowHelpNotification("~p~Descriptions~s~: Appuyez sur ~INPUT_CONTEXT~ pour intéragir")
            ...
            if DistanceDeletePoint < 3 then
                ...
                if IsControlJustPressed(1,51) then
                    if IsPedSittingInAnyVehicle(PlayerPedId()) then
                        ...
                        ESX.ShowNotification('~p~Vous n\'êtes pas dans un véhicule')
                    end
                end
            end
        end
    ...
    while menu do
        ...
        RageUI.IsVisible(menu, function() 
            ...
            if GetEntityHealth(vehicle) == GetEntityMaxHealth(vehicle) then
                ...
                RageUI.Separator('↓ ~p~Votre véhicule est endomagé~s~ ↓')
