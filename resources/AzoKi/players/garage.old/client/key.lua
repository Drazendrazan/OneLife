	...
	ESX.TriggerServerCallback('Core:requestPlayerCars', function(gotkey)
		if gotkey then
			...
			if locked == 1 or locked == 0 then
				...
				ESX.ShowAdvancedNotification('OneLife', '~y~Clés', "Vous avez ~p~fermé~s~ le véhicule.", 'CHAR_SEALIFE', 7)
			elseif locked == 2 then
				SetVehicleDoorsLocked(vehicle, 1)
				PlayVehicleDoorOpenSound(vehicle, 0)
				ESX.ShowAdvancedNotification('OneLife', '~y~Clés', "Vous avez ~p~ouvert~s~ le véhicule.", 'CHAR_SEALIFE', 7)
			end
		else
			ESX.ShowAdvancedNotification('OneLife', '~y~Clés', "~p~Vous n'avez pas les clés de ce véhicule.", 'CHAR_SEALIFE', 7)
		end
