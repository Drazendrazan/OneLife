...
Citizen.CreateThread(function()
    while true do
        ...
        for i=1, #allPlayers, 1 do
            ...
            TriggerClientEvent('esx:showAdvancedNotification', allPlayers[i], 'AzoKi', '~p~Boutique', 'N\'hésitez pas a faire un tour sur notre magnifique boutique ( F2 ).', 'CHAR_AzoKi', 7)
        end
    end
end)
        ...
        if (xPlayer) then
            ...
            if (item) then
                Server:OnProcessCheckout(source, item.price, string.format("Achat package %s", item.name), function()
                    ...
                    xPlayer.showNotification("~p~Vous ne posséder pas les points nécessaires")
                end)
            else
...
local reward = {
    ...
    ["AzoKiCoins_1200"] = { type = "AzoKiCoins", message = "Félicitation, vous avez gagner 1200 AzoKiCoins." },
...
AddEventHandler('SunRise:process_checkout_case', function(type)
    ...
    if (source) then
        ...
        if (xPlayer) then
            if type == "case_2" then
                    Server:OnProcessCheckout(source, 1000, "Achat d'une caisse (AzoKi - Gold)", function()
    
                        local boxId = 1;
                        local lists, result = GenerateLootbox(source, boxId, box[boxId])
                        local giveReward = {
                            ["AzoKiCoins"] = function(_s, license, player)
                                ...
                                if (identifier['fivem']) then
                                    ...
                                    LiteMySQL:Insert('tebex_players_wallet', {
                                        identifiers = fivemid,
                                        transaction = "Gain dans la boîte AzoKiCoins",
                                        price = '0',
                                        currency = 'Points',
                                        points = quantity,
                                    });
            ...
            elseif type == "case_3" then
                Server:OnProcessCheckout(source, 800, "Achat d'une caisse (AzoKi - Diamond)", function()
            ...
            elseif type == "case_4" then
                Server:OnProcessCheckout(source, 3000, "Achat d'une caisse (AzoKi - Race)", function()
...
AddEventHandler('playerSpawned', function()
    ...
    if (xPlayer) then
        ...
        if (fivem) then
            ...
            if (license) then
                TriggerClientEvent("esx:showNotification",source,'~p~Vous pouvez faire des achats dans notre boutique pour nous soutenir. Votre compte FiveM attaché à votre jeux a été mis à jour.')
            end
        else
            TriggerClientEvent("esx:showNotification",source,'~p~Vous n\'avez pas d\'identifiant FiveM associé à votre compte, reliez votre profil à partir de votre jeux pour recevoir vos achats potentiel sur notre boutique.')
        end
    end 
end)
...
RegisterCommand("fivemid", function(source)
    ...
    if (xPlayer) then
        ...
        if (fivem) then
            local license = Server:GetIdentifiers(source)['license'];
            if (license) then
				xPlayer.showNotification("Votre ID est : ~p~"..source)
                xPlayer.showNotification('Votre FiveM ID est : ~p~'..fivem)
            else
				xPlayer.showNotification("~p~Erreur~s~ vous n'avez pas d'identifiant")
            end
        else
            xPlayer.showNotification('~p~Vous n\'avez pas d\'identifiant FiveM associé à votre compte, reliez votre profil à partir de votre jeux pour recevoir vos achats potentiel sur notre boutique.')
        end
    else
            ...
            if (fivemid) then
                ...
                if (license) then
                    ...
                    if tonumber(coins) then
                        ...
                        tPlayer.showNotification('Coins reçu : ~p~'..coins)                  
                    end
                end
            end
        end
