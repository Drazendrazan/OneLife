...
RMenu.Add('case', 'main', AzoKiUI.CreateMenu("", ""))
RMenu:Get('case', 'main'):SetSubtitle("Caisse de ~p~OneLife")
...
AddEventHandler('tebex:on-open-case', function(animations, name, message)
    ...
    Citizen.CreateThread(function()
        ...
        for k, v in pairs(animations) do
            picture = v.name
            AzoKiUI.PlaySound("HUD_FREEMODE_SOUNDSET", "NAV_UP_DOWN")
            if v.time == 5000 then
                AzoKiUI.PlaySound("HUD_AWARDS", "FLIGHT_SCHOOL_LESSON_PASSED")
...
function openVisualCase()
    if openedVisualCase then
        AzoKiUI.Visible(RMenu:Get('case', 'main'), false)
        openedVisualCase = false
        return
    else
        openedVisualCase = true
        AzoKiUI.Visible(RMenu:Get('case', 'main'), true)
        Citizen.CreateThread(function()
            while openedVisualCase do
                Citizen.Wait(1.0)
                AzoKiUI.IsVisible(RMenu:Get('case', 'main'), true, false, true, function()
                    if (picture) then
                        AzoKiUI.RenderCaisse("case", picture)
                    end
                end)
            end
        end)
