...
AzoKi.netRegisterAndHandle("robberiesOpenMenu", function(id, active, difficulty)
    ...
    AzoKi.newThread(function()
        while menuIsOpened do
            ...
            RageUI.IsVisible(RMenu:Get(cat, sub("main")), true, true, true, function()
                tick()
                if active then
                    RageUI.Separator(("~s~↓ ~b~Rue~s~: ~o~%s ~s~↓"):format(streetName))
                    if exports.AzoKi:GetLevel() >= 5 then
                        RageUI.ButtonWithStyle("Crocheter la serrure", "Vous permets de tenter de crocheter la serrure et de pénetrer dans la propriétée si ce crochetage se solde par une réussite", {RightLabel = "→→"}, true, function(_,_,s)
                            if s then
                                AzoKiClientUtils.toServer("robberiesStart", id)
                                shouldStayOpened = false
                            end
                        end)
                    else
                        RageUI.Separator('~b~Tu n\'ai pas niveaux 5')
                    end
                end
            end, function()
                if active then
