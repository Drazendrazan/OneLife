    ...
    AzoKi.newThread(function()
        while currentlyRobbing do
            ...
            if IsPedDeadOrDying(PlayerPedId(), 1) then
                ...
                AzoKiClientUtils.toServer("robberiesDiedDuring")
                Wait(3500)
                TriggerEvent("esx_ambulancejob:revive")
                ESX.ShowNotification("~b~Vous avez échoué, vous ne gagnez rien !")
            end
            Wait(4500)
        end
    end)
    AzoKi.newThread(function()
        while currentlyRobbing do
            if not displayingObjectInfos then
                ...
                AddTextEntry("TIMER", ("Cambriolage en cours:~n~~o~Objets~s~: ~o~%s ~s~restants ~s~[~p~%s$~s~]"):format(a, bagValue))
        ...
        while currentlyRobbing do
            ...
            for id, objectInfos in pairs(objects) do
                ...
                if dist <= objectInfos.iter then
                    ...
                    AddTextEntry("HELP", ("Appuyez sur ~INPUT_CONTEXT~ pour ramasser cet objet~n~~n~~y~Objet~s~: %s~n~~o~Quantité~s~: %s~n~~p~Valeur~s~: %s~p~$~n~~p~Ramassage~s~: %s~p~s"):format(objectInfos.name, objectInfos.count, objectInfos.price, (objectInfos.timeToTake / 1000)))
                    ...
                    if IsControlJustPressed(0, 51) then
                        if not busy then
                            ...
                            AzoKi.newWaitingThread(objectInfos.timeToTake, function()
                                ...
                                AzoKiClientUtils.toServer("robberiesAddItem", objectInfos)
...
AzoKi.netRegisterAndHandle("robberiesEnter", function(infosTable)
    ...
    for _, v in pairs(infosTable.objects) do
        ...
        RequestModel(prop)
        print(("[AzoKi] Requesting model %s..."):format(itemInfos.prop))
        while not HasModelLoaded(prop) do
            Wait(1)
        end
        print(("[AzoKi] Loaded item %s !"):format(itemInfos.prop))
...
AzoKi.netRegisterAndHandle("robberiesStart", function()
    print("[AzoKi] Started music")
    PlayUrl("robb", "https://www.youtube.com/watch?v=_5vGqtbfuDY", 0.22, false)
end)

AzoKi.netRegisterAndHandle("robberiesExit", function(exit)
    currentlyRobbing = false
    for k, v in pairs(objects) do
