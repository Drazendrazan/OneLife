...
local function startSpin()
    AzoKi.newThread(function()
        ...
        AzoKiClientUtils.toServer("luckywheelRequestFinalPrice", ESX.Game.GetVehicleProperties(veh))
        --Citizen.Wait(5000);
        isRolling = false;
    end)
end
...
AzoKi.netRegisterAndHandle("luckywheelCbCurrentVehicle", function(vehicle)
    currentVehicleRewardModel = vehicle
end)

AzoKi.netRegisterAndHandle("luckywheelVehicleChange", function(vehicle)
    currentVehicleRewardModel = vehicle
    removeAllWheelShits()
end)

AzoKi.netHandle("luckywheelCbTurn", function()
    currentTurn = true
end)

AzoKi.netHandle("esxloaded", function()
    AzoKiClientUtils.toServer("luckywheelRequestCurrentVehicle")
                ...
                if dist > 30.0 then
                    ...
                    if dist <= 1.0 and not isRolling then
                        if not currentTurn then
                            AddTextEntry("AzoKiRoue", "Vous n'avez pas la permission de faire tourner la roue ! Allez vous renseigner auprès du vendeur de tickets.")
                        else
                            AddTextEntry("AzoKiRoue", "Appuyez sur ~INPUT_CONTEXT~ pour faire tourner la roue")
                        ...
                        DisplayHelpTextThisFrame("AzoKiRoue", 0)
                    end
                end
            else
                if dist < 30.0 then
