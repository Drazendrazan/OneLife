...
local color = {200, 0, 0, 200 } -- Text color
...
local function Display(ped, text)
    ...
    if dist <= distToDraw then
        ...
        local display = true
        AzoKi.newWaitingThread(displayTime, function()
            display = false
        end)
...
RegisterCommand('me', function(source, args)
    if allowed then
        ...
        AzoKiClientUtils.toServer("shareDisplay", idsToSend, text, pPedSID)
        ActionMeCoolDown()
    end
end)
...
AzoKi.netRegisterAndHandle('shareDisplay', function(text, serverId)
...
function ActionMeCoolDown()
    allowed = false
    AzoKi.newWaitingThread(6*1000, function()
        allowed = true
    end)
end

AzoKi.netRegisterAndHandle('adminTag:trigger', function(sender, message)
