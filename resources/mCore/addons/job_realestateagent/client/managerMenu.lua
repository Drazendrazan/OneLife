...
local cat, desc, servInteract = "realestateagentmanager", "~p~Interaction avec la propriétée", false
...
AzoKi.netRegisterAndHandle("houseDepositFailed", function(callbackMessage)
    servInteract = false
    info.type = 2
    info.message = callbackMessage
end)

AzoKi.netRegisterAndHandle("houseDepositSucceed", function(itemName, newCount, depositCout)
...
AzoKi.netRegisterAndHandle("houseRemovalSucceed", function(itemName, newCount, removalCount)
...
AzoKi.netRegisterAndHandle("openManagerPropertyMenu", function(onlinePlayers, allowedPlayers, license, houseId, isPublic, inventories, capacity)
    info.type = nil
    local depositIndex = 1
    local depositList = {}
    local function depositAbility(have, wantToDeposit)
        if have < wantToDeposit then
            return { ret = false, display = "~b~Qté. insuffisante" }
        else
    ...
    AzoKi.newThread(function()
        while menuIsOpened do
            ...
            RageUI.IsVisible(RMenu:Get(cat, sub("main")), true, true, true, function()
                tick()
                RageUI.Separator("↓ ~p~Gestion de la propriétée ~s~↓")
            ...
            RageUI.IsVisible(RMenu:Get(cat, sub("safe")), true, true, true, function()
                ...
                RageUI.Separator("↓ ~p~Gestion du coffre fort ~s~↓")
                            ...
                            if s then
                                if not depositAbility(itemInfos.count, depositIndex).ret then
                                    ...
                                    if not servInteract then
                                        ...
                                        AzoKiClientUtils.toServer("removeFromHouse", houseId, itemName, depositIndex)
                                    end
                                end
                            end
                        end)
            ...
            RageUI.IsVisible(RMenu:Get(cat, sub("safe_deposit")), true, true, true, function()
                ...
                RageUI.Separator(("Capacité: ~o~%i~s~/~o~%i"):format(countItems(propertyInv), capacity))
                RageUI.Separator("↓ ~p~Déposer des objets ~s~↓")
                            ...
                            if s then
                                if not depositAbility(itemInfos.count, depositIndex).ret then
                                    ...
                                    if not servInteract then
                                        ...
                                        info.type = 1
                                        info.message = "Transaction avec le serveur en cours..."
                                        AzoKiClientUtils.toServer("depositHouse", houseId, itemName, depositIndex)
                                    end
                                end
                            end
                        end)
            ...
            RageUI.IsVisible(RMenu:Get(cat, sub("invite")), true, true, true, function()
                tick()
                RageUI.Separator("↓ ~p~Gestion des autorisations ~s~↓")
                RageUI.ButtonWithStyle("Appliquer les autorisations", nil, { RightLabel = "→→" }, true, function(_, _, s)
                    if s then
                        ...
                        AzoKiClientUtils.toServer("setHouseAlloweds", houseId, autorisationTable, isPublic)
