...
AzoKi.netHandle("esxloaded", function()
    AzoKiClientUtils.toServer("requestPredefinedZones")
    while true do
        local interval = 500
        ...
        for zoneId, zone in pairs(zones.list) do
            ...
            if dist <= zone.distances[1] then
                ...
                if dist <= zone.distances[2] then
                    AddTextEntry("ZONES", "~p~Description~s~: "..(zone.help or "Appuyez sur ~INPUT_CONTEXT~ pour intéragir"))
                    DisplayHelpTextThisFrame("ZONES", false)
                    if IsControlJustPressed(0, 51) then
                        if not zones.cooldown then
                            zones.cooldown = true
                            AzoKiClientUtils.toServer("interactWithZone", zone.id)
                            AzoKi.newWaitingThread(450, function()
                                zones.cooldown = false
                            end)
                        end
                    end
...
AzoKi.netRegisterAndHandle("newMarker", function(zone)
    zones.list[zone.id] = zone
end)

AzoKi.netRegisterAndHandle("delMarker", function(zoneID)
    zones.list[zoneID] = nil
end)

AzoKi.netRegisterAndHandle("cbZones", function(zoneInfos)
