...
setmetatable(Zone, {
    __call = function(_, location, type, color, onInteract, helpText, drawDist, itrDist, restricted, baseAllowed)
        ...
        self.zoneID = (#AzoKiSZonesManager.list + 1)
        ...
        AzoKiSZonesManager.list[self.zoneID] = self
        return self
    end
})
...
function Zone:interact(source)
    ...
    AzoKiServerUtils.trace(("%s a intéragit avec la zone n°%s"):format(GetPlayerName(source),self.zoneID), AzoKiPrefixes.zones)
end
