...
setmetatable(Npc, {
    __call = function(_, model, ai, frozen, position, animation, restricted, allowed)
        ...
        self.id = #AzoKiSNpcsManager.list + 1
        self.model = model
        self.ai = ai
        self.frozen = frozen
        ...
        AzoKiSNpcsManager.list[self.id] = self
        return self;
    end
})
...
function Npc:playSpeech(speech, param)
    if self:isRestricted() then
        for source, _ in pairs(self.allowed) do
            AzoKiServerUtils.toClient("npcPlaySound", source, self.id, speech, param)
        end
    else
        AzoKiServerUtils.toAll("npcPlaySound", self.id, speech, param)
    end
end
