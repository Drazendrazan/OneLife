...
AzoKi.netHandle("esxloaded", function()
    AzoKiClientUtils.toServer("requestPredefinedNpcs")
            ...
            for npcId, npc in pairs(npcs.list) do
                ...
                if not npcs.list[npcId].npcHandle then
                    if #(pos - npcPos) <= genDist then
                        ...
                        if npc.frozen then
                            AzoKi.newWaitingThread(1500, function()
...
AzoKi.netRegisterAndHandle("npcPlaySound", function(npcId, speech, param)
...
---@param npc Npc
AzoKi.netRegisterAndHandle("newNpc", function(npc)
    npcs.list[npc.id] = npc
end)

AzoKi.netRegisterAndHandle("delNpc", function(npcId)
...
AzoKi.netRegisterAndHandle("cbNpcs", function(incomingNpcs)
