...
local Licenses = {}
local permisencours = "drive"

function StopDriveTest(success)
	if success then
		TriggerServerEvent('</eDen:addLicense', permisencours)
		RemoveBlip(CurrentBlip)
		ESX.ShowAdvancedNotification(eDAutoEcole.AutoEcole.Infos.NomDuMonitor, '~b~Bravo~w~', 'Vous avez reçu votre permis !', eDAutoEcole.AutoEcole.Infos.CHAR)
		...
		ESX.ShowAdvancedNotification(eDAutoEcole.AutoEcole.Infos.NomDuMonitor, '~b~Malheureusement~w~', 'Vous avez raté le test !', eDAutoEcole.AutoEcole.Infos.CHAR)			
		if DoesEntityExist(GetVehiclePedIsIn(PlayerPedId(), false)) then
			DeleteEntity(GetVehiclePedIsIn(PlayerPedId(), false))
			SetVehicleAsNoLongerNeeded(GetVehiclePedIsIn(PlayerPedId(), false))
		end
		...
		if CurrentTest == 'drive' then
			...
			if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
				...
				if ErrorSpeed == 0 and GetSpeed >= speed_limit_residence then
					...
					ESX.ShowAdvancedNotification('Amanda', '~b~Vous avez fait une erreur~w~', "Vous roulez trop vite, vitesse limite : " ..speed_limit_residence.. " km/h~w~!\n~b~Nombre d'erreurs " ..DriveErrors.. "/5", eDAutoEcole.AutoEcole.Infos.CHAR)
					Wait(2000) -- evite bug
				end
				...
				if ErrorSpeed == 1 and GetSpeed >= speed_limit_ville then
					...
					DriveErrors       = DriveErrors + 1
					IsAboveSpeedLimit = true
					ESX.ShowAdvancedNotification('Amanda', '~b~Vous avez fait une erreur~w~', "Vous roulez trop vite, vitesse limite : " ..speed_limit_ville.. " km/h~w~!\n~b~Nombre d'erreurs " ..DriveErrors.. "/5", eDAutoEcole.AutoEcole.Infos.CHAR)
					Wait(2000)
				end
				...
				if ErrorSpeed == 2 and GetSpeed >= speed_limit_otoroute then
					...
					DriveErrors       = DriveErrors + 1
					IsAboveSpeedLimit = true
					ESX.ShowAdvancedNotification('Amanda', '~b~Vous avez fait une erreur~w~', "Vous roulez trop vite, vitesse limite : " ..speed_limit_otoroute.. " km/h~w~!\n~b~Nombre d'erreurs " ..DriveErrors.. "/5", eDAutoEcole.AutoEcole.Infos.CHAR)
					Wait(2000)
				end

				if HasEntityCollidedWithAnything(vehicle) and DamageControl == 0 then
					DriveErrors       = DriveErrors + 1
					ESX.ShowAdvancedNotification(eDAutoEcole.AutoEcole.Infos.NomDuMonitor, '~b~Vous avez fait une erreur~w~', "Votre vehicule c\'est pris des dégats\n~b~Nombre d'erreurs " ..DriveErrors.. "/5", eDAutoEcole.AutoEcole.Infos.CHAR)
					Wait(2000)
				end
				...
				if GetEntityHealth(vehicle) < GetEntityHealth(vehicle) then
					...
					ESX.ShowAdvancedNotification('Amanda', '~b~Vous avez fait une erreur~w~', "Votre vehicule c\'est pris des dégats\n~b~Nombre d'erreurs " ..DriveErrors.. "/5", eDAutoEcole.AutoEcole.Infos.CHAR)
...
function StartDriveTest()
    for k, v in pairs(eDAutoEcole.AutoEcole.DriveTeste.DriveTestVoiture) do
        ...
        ESX.ShowAdvancedNotification(eDAutoEcole.AutoEcole.Infos.NomDuMonitor, '~b~Me voila !~w~', 'Tenez votre voiture, bonne route et bonne chance !', eDAutoEcole.AutoEcole.Infos.CHAR)
...
function StartDriveTestMoto()
    for k, v in pairs(eDAutoEcole.AutoEcole.DriveTeste.DriveTestMoto) do
        ...
        ESX.ShowAdvancedNotification(eDAutoEcole.AutoEcole.Infos.NomDuMonitor, '~b~Me voila !~w~', 'Tenez votre moto, bonne route et bonne chance !', eDAutoEcole.AutoEcole.Infos.CHAR)
...
function StartDriveTestCamion()
    for k, v in pairs(eDAutoEcole.AutoEcole.DriveTeste.DriveTestCamion) do
        ...
        ESX.ShowAdvancedNotification(eDAutoEcole.AutoEcole.Infos.NomDuMonitor, '~b~Me voila !~w~', 'Tenez votre camion, bonne route et bonne chance !', eDAutoEcole.AutoEcole.Infos.CHAR)
